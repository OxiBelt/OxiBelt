...
`tls_provider` selects the rustls crypto provider used by downstream TLS, upstream TLS, HTTP/3, Admin QUIC, TURN TLS, ticket encryption, certificate verification, and QUIC client/server configuration. The default is `aws_lc_rs`. `ring` requires an OxiBelt build with the `crypto-ring` Cargo feature. The `ring` provider does not support the post-quantum hybrid `x25519mlkem768` or RFC 10024 `secp256r1mlkem768` TLS 1.3 key exchange groups, so configurations that set `tls_provider = "ring"` must omit the former from global and route-specific TLS 1.3 negotiation policies and must not enable the latter on any TLS surface. Upstream ECH requires `aws_lc_rs`. This selection and RFC 10024 support do not make a FIPS validation claim.

`[crypto.auxiliary_tls].enable_secp256r1mlkem768` controls RFC 10024 SecP256r1MLKEM768 for OxiBelt-owned auxiliary TLS clients only. It defaults to `false`, retains the existing provider order when absent, and, when enabled, retains `x25519mlkem768` before `secp256r1mlkem768`. It applies to control-plane HTTP such as external authentication, live OCSP/CRLite/CT downloads, external cache handlers, and HTTPS OTLP access-log export. Discovery requests instead use the independent discovery entry's TLS control. It does not change downstream, named upstream, Admin, or Redis TLS policy, and it is restart-only. SQLx TLS and dependency-owned Kubernetes TLS remain outside this configuration surface and keep their dependency-owned policy.

Standalone tools use separate opt-in flags: `oxibeltctl --admin-tls-secp256r1mlkem768` applies to its Admin connection, `oxibeltctl --auxiliary-tls-secp256r1mlkem768` applies to its owned auxiliary HTTPS downloads, and the Gateway controller uses `--auxiliary-tls-secp256r1mlkem768` for its owned Kubernetes API control client. Helm exposes the same disabled defaults through `auxiliaryTls.enableSecp256r1Mlkem768` in the Gateway-controller chart and `supplyChainAdmission.webhook.enableSecp256r1Mlkem768` for the admission webhook, whose standalone flag is `oxibeltctl supply-chain admission-server --tls-secp256r1mlkem768`.
...
[tls.1_3]
# RFC 10024 SecP256r1MLKEM768 is excluded by default.
...
`tls.ssl_early_data` controls accepted downstream TLS early data. The default is `off`. `safe_methods` permits transport-verified `GET`, `HEAD`, and exact-uppercase `QUERY` requests, while `on` permits all methods and should be used only for routes that tolerate replay. TCP TLS early data requires TLS 1.3 and `tls.resumption.mode = "stateful"`; with `[[tls.certificates]]`, it also requires `tls.resumption.multi_certificate = "partition_by_sni"` plus strict SNI. HTTP/3 0-RTT transport admission remains controlled by `quic.zero_rtt`; `zero_rtt = "safe_methods"` is the recommended mode. When QUIC admits early data, the effective `ssl_early_data` mode controls route and method policy after route matching. Client-supplied `Early-Data` headers are stripped; OxiBelt adds `Early-Data: 1` upstream only for transport-verified early-data requests.

`tls.1_3.key_exchange_groups` controls the downstream TCP TLS, HTTP/3, and TURN TLS named groups exposed through the aws-lc-rs provider. The TLS 1.3 default keeps rustls' post-quantum hybrid first: `["x25519mlkem768", "x25519", "secp256r1", "secp384r1"]`. RFC 10024 `secp256r1mlkem768` is supported but excluded from that default; add it to this list, or to an exact-SNI route list, to opt in only on the selected downstream surface. It is TLS 1.3-only. The legacy flat `tls.key_exchange_groups` key remains accepted as a compatibility alias for `tls.1_3.key_exchange_groups` only; it no longer changes TLS 1.2 behavior.
...
`retry = true` enables QUIC Retry/address validation for unvalidated downstream HTTP/3 connection attempts. `zero_rtt = "safe_methods"` enables QUIC TLS early data and rejects unsafe requests that the QUIC transport reports as early data with `425 Too Early`; only early-data `GET`, `HEAD`, and exact-uppercase `QUERY` are accepted.
...
[[rate_limits]]
name = "per-ip"
# policy_id = "per-ip" # optional: advertise RateLimit and RateLimit-Policy
key = "client_ip"
rate = "10r/s"
burst = 50
max_buckets = 16384
...
Set `policy_id` on an enforcing top-level `[[rate_limits]]` entry to opt into downstream [RateLimit and RateLimit-Policy fields](https://datatracker.ietf.org/doc/draft-ietf-httpapi-ratelimit-headers/). Existing entries without `policy_id` remain silent. IDs must be unique, contain 1–64 ASCII letters, digits, `.`, `_`, or `-`, and no more than 16 policies may opt in. `policy_id` is rejected in monitor mode or when the configured rate does not yield a finite positive per-second refill. A public field identifies the policy as `oxibelt/<policy_id>` and reports the effective bucket capacity (`q`), remaining whole tokens after the decision (`r`), and time to the next token (`t`) when exhausted and available. Continuous token refill has no fixed window, so OxiBelt omits `w`; it also omits `pk` so client identity is not disclosed. Limits checked before an early denial may appear, while unchecked limits do not. Responses with `Retry-After` omit local `t`. OxiBelt appends distinct local policies to valid upstream fields and suppresses local fields if upstream field parsing is unsafe or an upstream item uses the same policy ID. Generated fields are omitted on cache hits; identity-specific advertised balances add downstream `Cache-Control: private` after cache handling. Treat the field as operational disclosure: use nonsecret IDs and enable it only on limits whose capacity and remaining budget clients may observe. Existing enforcement and failure policies are unchanged.
...
`[circuit_breakers]` is enabled by default. It is a process-local guard that is independent of `[overload]`: normal work is admitted only while the global and configured route/pool limits have capacity, and bounded FIFO queues reject rather than accumulating unbounded waiters. `enabled = false` is a compatibility escape hatch that restores the pre-breaker admission behavior. Circuit-breaker rejections use the configured `5xx` response (default `503`) and `Retry-After`, preserve reusable HTTP/1 connections, and never make readiness fail. The exception is a valid `Incremental: ?1` request rejected for an active limit, full queue, or queue timeout: it receives `429`, `Retry-After`, `Cache-Control: no-store`, and mandatory `Proxy-Status: oxibelt; error=connection_limit_reached`; these marked capacity rejections always close HTTP/1.x connections. Circuit-open, retry-budget, internal-state-unavailable, and unmarked-request behavior is unchanged; see [temporary capacity rejection](Incremental.md#temporary-capacity-rejection).
...
[admin.tls]
enabled = false
enable_secp256r1mlkem768 = false # RFC 10024; TLS 1.3 only
min_version = "tls1.3"
max_version = "tls1.3"
...
`no_vary_search = true` enables the current draft-09 automatic [No-Vary-Search cache semantics](NoVarySearch.md) for GET, HEAD, and opt-in QUERY across L1, shared L2, and optional external L3. OxiBelt tries the exact query representation first, then uses a bounded indexed equivalent only when the response declaration covers both the received and effective query representations. Explicit `{query:name}` key tokens remain part of the configured key material, and the configured raw `partition_key` remains an independent cache boundary; No-Vary-Search does not rewrite either template. Response-WAF routes remain exact-only. L2 and L3 equivalent lookup requires the bounded index path; a legacy L3 handler without the optional capability remains exact-only. Set this top-level switch to `false` to disable equivalent alias lookup and storage while retaining ordinary forwarding and invalidation behavior.

`[cache.groups]` is enabled by default. Set `enabled = false` to retain the legacy cache identity globally, or set `[cache.policies.groups] enabled = false` for one named cache policy. Group-aware responses use the RFC 9875 `Cache-Groups` and `Cache-Group-Invalidation` Structured Fields lists: only string members are accepted, with 64 raw members, 256 decoded bytes per member, and 16 KiB combined field data. The optional nested policy table has only `enabled`; unknown settings are rejected. See [CacheGroups.md](CacheGroups.md) for invalidation, Admin authorization, shared-state recovery, and rollout behavior.

`[[cache.external_handlers]]` defines optional HTTP L3 handlers behind local L1 and shared-state L2. `cache.external_handler = "name"` selects a top-level default, `cache.policies.external_handler = "name"` overrides it, and `cache.policies.external_handler = "off"` disables inherited L3 for that policy. Handler names must be unique runtime identifiers and cannot be `default` or `off`; endpoints must be `http://` or `https://`; timeout, metadata, body, and inflight limits must be positive. `request_timeout_ms` bounds each handler request/response exchange, including lookup and purge response-body reads. `token_env` supplies a bearer token without putting credentials in the TOML. The handler protocol uses JSON control requests for lookup, revalidation metadata refresh, and purge, plus framed cache entries with an 8-byte big-endian metadata length, UTF-8 JSON metadata, and raw body bytes. OxiBelt remains authoritative for keys, partitions, `Vary`, No-Vary-Search equivalence, cacheability, credential bypasses, status headers, admission, and purge authorization. L3 equivalent lookup and storage require the optional No-Vary-Search capability; handlers without it preserve exact-key GET/HEAD/QUERY behavior. Handler errors, malformed records, mismatches, and expired records are safe misses under `local_only`.

Exact-uppercase `QUERY` can be added to `cache_methods`; the default remains `["GET", "HEAD"]`. QUERY requires a valid Content-Type and uses a separate body-bound key namespace. See [QUERY admission, capture, cache, and retry behavior](Query.md) before enabling it.

`[cache.query_cleanup]` bounds asynchronous physical cleanup after QUERY invalidation. `queue_capacity` sets the total outstanding-target limit across queued, pending, and active cleanup (`1..=1024`, default `64`), `batch_size` limits entries removed per cleanup operation (`1..=512`, default `128`), and `max_concurrent` limits cleanup operations running at once (`1..=4`, default `1`). External handlers receive at most four bounded cleanup exchanges for one invalidation; incomplete physical cleanup then relies on entry expiry while the authoritative epoch continues to reject stale records.
...
`[cache.surrogate]` parses `Surrogate-Control` for `no-store`, `max-age`, `stale-if-error`, and `stale-while-revalidate`. When enabled, those directives control OxiBelt cache metadata ahead of origin `Cache-Control`, and `strip_response_header = true` removes `Surrogate-Control` before downstream delivery and cached hits. `tag_headers` extracts whitespace- or comma-separated cache tags from response headers such as `Surrogate-Key` and `Cache-Tag`; admin tag purge can remove all entries carrying a tag. `background_refresh` serves a stale response immediately during `stale-while-revalidate` and refreshes eligible GET/HEAD and opt-in QUERY responses in the background. QUERY refresh retains the effective content and trailers and supports HTTP/3 upstreams. OxiBelt skips background refresh for response-WAF inspected routes, non-QUERY HTTP/3 upstreams, and PROXY protocol egress routes, which continue to use foreground revalidation. `lock_wait_timeout_ms` bounds local collapsed-forwarding followers and shared fill-conflict polling so a stuck fill cannot block indefinitely.
...
`admin.tls.enable_secp256r1mlkem768` independently opts Admin TCP TLS and Admin HTTP/3 into RFC 10024 SecP256r1MLKEM768. It defaults to `false`, requires Admin TLS to permit TLS 1.3, and is restart-only.
...
[upstreams.tls]
enable_secp256r1mlkem768 = false # RFC 10024; independent opt-in
...
TLS client-certificate route matchers can check `present`, `fingerprint_sha256`, `subject_cn`, `san_dns`, and `san_ip`. TCP TLS and HTTP/3/WebTransport populate this metadata from the downstream client certificate accepted by the TLS stack. Verified HTTP/3 certificates can match these selectors. When client-certificate metadata is missing or incomplete, HTTP/3/WebTransport certificate-specific matchers still fail closed, including `present = false`. Reserved-capacity mTLS eligibility remains TCP-only. OxiRule additionally exposes bounded leaf metadata for downstream clients and upstream servers; see [Peer certificate metadata](OxiRule.md#peer-certificate-metadata).

`routes.tls.ssl_early_data` overrides the global `tls.ssl_early_data` mode for requests resolved to that route. Use `off` for replay-sensitive routes, `safe_methods` for safe `GET`/`HEAD`/`QUERY` handling, and `on` only when every accepted method on the route is replay-safe by application design. `routes.tls.min_version` and `routes.tls.max_version` override downstream TCP TLS protocol versions by SNI for exact-host root routes and inherit omitted values from `[tls]`. If a request's HTTP Host resolves to a route with a different TLS negotiation policy than the SNI-selected policy, OxiBelt returns `421 Misdirected Request`.
