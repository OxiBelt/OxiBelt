...
`routes.tls.ssl_early_data` overrides the global `tls.ssl_early_data` mode for requests resolved to that route. Use `off` for replay-sensitive routes, `safe_methods` for idempotent `GET`/`HEAD` handling, and `on` only when every accepted method on the route is replay-safe by application design. `routes.tls.min_version` and `routes.tls.max_version` override downstream TCP TLS protocol versions by SNI for exact-host root routes and inherit omitted values from `[tls]`.
