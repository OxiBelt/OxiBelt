...
`[cache.query_cleanup]` bounds asynchronous physical cleanup after QUERY invalidation. `queue_capacity` sets the pending-target limit (`1..=1024`, default `64`), `batch_size` limits entries removed per cleanup operation (`1..=512`, default `128`), and `max_concurrent` limits cleanup operations running at once (`1..=4`, default `1`).
