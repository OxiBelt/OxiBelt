...
Static hot-object caching is opt-in. Set `open_file_cache_max_entries`, `open_file_cache_ttl_ms`, and `hot_object_cache_max_bytes` to enable a bounded TTL cache for verified small static responses. `open_file_cache_max_entries` and `open_file_cache_ttl_ms` bound the entry count and freshness window; `hot_object_cache_max_bytes` and `hot_object_cache_max_file_bytes` bound body memory globally and per file. Cache fill and expiry refresh still open the file through the secure static-root resolution path, and expired entries refresh only when the current validator matches the cached object. Fresh cached hits preserve validators and range behavior while serving the cached body until `open_file_cache_ttl_ms` expires. Deleted, inaccessible, or replaced files may therefore continue to serve the cached safe body inside that TTL window, then refresh or fail closed from the current filesystem state after expiry; use `0` values to keep the default no-cache behavior.
