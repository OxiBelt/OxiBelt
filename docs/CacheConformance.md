...
| QUERY | Opt-in RFC 10008 caching binds complete original and effective content, metadata, and trailers; supports conditional/range hits and invalidation fences. | Rust `query` tests; Docker `cache/query-protocol-cache-matrix` exercises nine downstream/upstream protocol combinations. |
| QUERY with Cache-Groups | Unsafe responses advance QUERY epochs and reclaim obsolete indexed entries with groups enabled; group invalidation remains independently enforced. | Rust group-enabled QUERY tests; Docker `shared-state/redis-valkey-cluster-state` and `shared-state/postgres-cluster-state`. |
| Group-aware Admin operations | Warming retains the group request scope when verifying storage. Group-aware purges count logical representations across tiers and preserve redacted audit events. | Rust warm and shared-cache tests; Docker `cache/json-warming` and `cache/purge-audit`. |
