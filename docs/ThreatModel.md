...
Release CI also creates and verifies GitHub API-hosted keyless SLSA provenance and
...
| `gateway-api-udproute` | Attacker-selected UDP peers can consume flow-table and socket resources. Bounded new-flow/datagram admission, idle expiry, capacity accounting, and Pod-local lifecycle reduce but do not eliminate amplification and state-exhaustion risk. |
