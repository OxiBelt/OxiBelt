...
| `supply-chain-admission-bundle` | GitHub-hosted provenance, SBOMs, rebuild receipts, release metadata, revocation policy, workload-policy input, and operator approval are untrusted evidence inputs. Bounded canonical parsing and exact repository, workflow, source revision, role, architecture, digest, freshness, and verifier-revision binding precede deterministic Ed25519 signing. Bundle v2 also binds every approved auxiliary's semantic class, Kubernetes name, and exact digest; v1 authorizes only the primary. The credential-free webhook rechecks signature, expiry, revocation, workload annotation, role, and the complete final regular/init/native-sidecar/ephemeral image set with `failurePolicy: Fail`. Auxiliary approval proves signer intent, not OxiBelt provenance or the safety of command, arguments, environment, mounts, capabilities, or security context. Compromise of the signing key, approved verifier, GitHub evidence authority, webhook image/TLS key, API server, or cluster admission path can subvert or deny deployment, while webhook unavailability intentionally denies admission. Live TLS rotation, admission availability, and CNI qualification remain incomplete promotion evidence. |
