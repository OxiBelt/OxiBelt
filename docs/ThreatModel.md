...
| --- | --- |
| `http-resumable-uploads` | Negotiated draft-12/interop-9 relay preserves informational-response ordering and absolute deadlines without implying `Incremental` or allowing automatic replay. Managed uploads are opt-in: operator-owned profiles bind destinations, stores, verified owners, inspection requirements, and finite quotas. Complete parts and assembled content must pass WAF inspection before durable offsets or delivery; uncertain upstream delivery is terminal and is not replayed. Local storage requires a private single-instance root; shared storage trusts PostgreSQL fencing and the configured TLS-verified, unversioned S3 bucket. Compromised identity providers, storage operators, database/object-store services, or upstream destinations remain trusted-boundary risks. Availability, retention cleanup, crash recovery, deployment isolation, and resource limits require deployment-specific qualification; the draft protocol and feature are not stable production guarantees. |
