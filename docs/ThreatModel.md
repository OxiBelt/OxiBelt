...
| `supply-chain-admission-bundle` | GitHub-hosted provenance, SBOMs, rebuild receipts, release metadata, revocation policy, and operator approval are untrusted evidence inputs. Bounded canonical parsing and exact repository, workflow, source revision, role, architecture, digest, freshness, and verifier-revision binding precede deterministic Ed25519 signing; the credential-free webhook rechecks bundle shape, signature, expiry, revocation, workload annotation, role, and image digest with `failurePolicy: Fail`. Compromise of the signing key, approved verifier, GitHub evidence authority, webhook image/TLS key, API server, or cluster admission path can subvert or deny deployment, while webhook unavailability intentionally denies admission. Live TLS rotation, admission availability, and CNI qualification remain incomplete promotion evidence. |
| `helm-data-plane` | Chart output depends on operator values and cluster controls and is not a complete security deployment attestation. V2 treats values, labels, mounts, dependency declarations, seccomp annotations, manifest digests, and bundle transport as untrusted deployment inputs; strict helper/schema validation, reserved selectors, typed writable volumes, default deny, digest-pinned role identity, and a signed exact-evidence bundle make intended authority reviewable. The credential-free webhook fails closed on signature, expiry, revocation, bundle annotation, role, or image mismatch. A compromised bundle signing key, chart release, admission controller/tools image/TLS key, ServiceAccount signer, API server, node/runtime, CNI, storage provider, registry, or operator can still subvert or deny deployment. The profile report cannot prove runtime-observed seccomp/Landlock, Secret contents, CNI enforcement, or that the signing authority made a sound approval decision. |
