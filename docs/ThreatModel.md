...
- Release workflows publish role-specific platform images and
  ...
  stable and beta releases. Bundle generation also requires the producer's
  exact canonical index metadata and selected SBOM hash, binds a positive
  canonical-branch workflow run attempt into each receipt, rereads the
  completed run after artifact downloads, and caps deployability at the
  earliest evidence freshness horizon. Equivalent duplicate attestations are
  selected by trusted timestamp and canonical object digest. The bundles are
  not GHCR OCI referrers. Historical registry referrers do not imply current
  API evidence coverage, and an existing fail-closed admission policy must be
  deliberately evaluated rather than weakened to unblock an image.
...
| --- | --- |
| `http-compression-dictionary` | Opt-in RFC 9842 uses separate origin, direction, profile, and policy scopes. Public provisioning and explicit fresh advertisements authorize dictionary use; credentials, private responses, and unverified transport do not. Decoders enforce hash binding and finite memory, expansion, concurrency, and time limits before WAF inspection. Purge and publication use generation fencing across local and shared stores. Managed uploads retain encoded offsets while fenced final validation publishes decoded objects. Storage services and operator configuration remain trusted; deployment capacity and performance require separate qualification. |
| `http-resumable-uploads` | Negotiated draft-12/interop-9 relay preserves informational-response ordering and absolute deadlines without implying `Incremental` or allowing automatic replay. Managed uploads are opt-in: operator-owned profiles bind destinations, stores, verified owners, inspection requirements, and finite quotas. Identity parts must pass WAF inspection before durable offsets; explicitly pinned dictionary uploads acknowledge bounded encoded staging and require decoded whole-body inspection before publication or delivery; uncertain upstream delivery is terminal and is not replayed. Local storage requires a private single-instance root; shared storage trusts PostgreSQL fencing and the configured TLS-verified, unversioned S3 bucket. Compromised identity providers, storage operators, database/object-store services, or upstream destinations remain trusted-boundary risks. Availability, retention cleanup, crash recovery, deployment isolation, and resource limits require deployment-specific qualification; the draft protocol and feature are not stable production guarantees. |
...
| `admin-staged-membership` | Current active members, PostgreSQL, signer/IPM authority, readiness keys, catch-up keys, and the shared artifact key remain trusted membership boundaries. Proposals, activations, and cancellations are serialized through the existing all-current-member protected mutation path; learners remain non-voting, catch-up evidence is recipient encrypted, readiness is independently signed, and a new epoch is authoritative only after durable committed activation. Loss of any required active member intentionally denies protected writes, compromised membership authority can redirect or fence the cluster, and emergency reconstitution remains out of band. Crash-point, partition, corruption, mixed-version, and live multi-node qualification remain incomplete promotion evidence. |
...
| `gateway-api-backendtlspolicy` | The implemented subset binds SNI to one precise hostname and authentication to either that hostname or an explicit bounded DNS/URI SAN set after WebPKI chain validation. System trust or a deterministic, content-addressed, 256-KiB aggregate of up to eight public ConfigMap CAs is allowed. A Gateway may independently select one operator-allowlisted core Secret through `spec.tls.backend.clientCertificateRef`; exact-name source GET/watch RBAC, any required `ReferenceGrant`, bounded certificate/key validation, immutable derived target Secrets, isolated mounts, and fail-closed conflicting attachments constrain that private-key boundary. Invalid client identities produce complete match-equivalent terminal `503` tombstones with no route-policy or upstream effects, preventing a narrower route from disappearing into a broader fallback; incomplete reconstruction, mixed translation errors, or incompatible rolling-upgrade targets preserve the last good artifact. Recovery atomically restores forwarding and the derived mount. Source private-key bytes are excluded from generated ConfigMaps, status, logs, explain output, and artifact digests. Referenced ConfigMap/Secret integrity and controller RBAC remain trust boundaries; unsupported options, Secret CA refs, unauthorized identities, and pin fields fail closed. |
...
| `supply-chain-admission-bundle` | GitHub-hosted provenance, SBOMs, rebuild receipts, release metadata, revocation policy, workload-policy input, and operator approval are untrusted evidence inputs. Bounded canonical parsing and exact repository, workflow, source revision, role, architecture, digest, freshness, and verifier-revision binding precede deterministic Ed25519 signing. Generation binds the producer's canonical index-metadata hash and selected SBOM hash, requires canonical-branch positive workflow-attempt identity in every rebuild receipt, rereads completed workflow state after downloads, and caps deployability at the earliest signed evidence horizon. Bundle v2 also binds every approved auxiliary's semantic class, Kubernetes name, and exact digest; v1 authorizes only the primary. The credential-free webhook rechecks signature, expiry, evidence freshness, revocation, workload annotation, role, and the complete final regular/init/native-sidecar/ephemeral image set with `failurePolicy: Fail`. Auxiliary approval proves signer intent, not OxiBelt provenance or the safety of command, arguments, environment, mounts, capabilities, or security context. Compromise of the signing key, approved verifier, GitHub evidence authority, webhook image/TLS key, API server, or cluster admission path can subvert or deny deployment, while webhook unavailability intentionally denies admission. Live TLS rotation, admission availability, and CNI qualification remain incomplete promotion evidence. |
| `helm-data-plane` | Chart output depends on operator values and cluster controls and is not a complete security deployment attestation. V2 treats values, labels, mounts, dependency declarations, seccomp annotations, manifest digests, and bundle transport as untrusted deployment inputs; strict helper/schema validation, reserved selectors, typed writable volumes, default deny, digest-pinned role identity, and a signed exact-evidence bundle make intended authority reviewable. Helm transports no unsigned auxiliary allowlist, and the credential-free webhook fails closed on signature, expiry, revocation, bundle annotation, role, any unapproved executable image, or unavailable exact Pod/ephemeral-subresource routing. The endpoint revision binds the bundle and fixed webhook image so different admission binaries are not served in one epoch. A compromised bundle signing key, chart release, admission controller/tools image/TLS key, ServiceAccount signer, API server, node/runtime, CNI, storage provider, registry, operator, or mutating webhook with authority to remove the release selector labels can still subvert or deny deployment. The profile report cannot prove runtime-observed seccomp/Landlock, Secret contents, CNI enforcement, full PodSpec integrity, or that the signing authority made a sound approval decision. |
