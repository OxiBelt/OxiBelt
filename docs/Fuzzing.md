...
AddressSanitizer and sustained profiles to `nightly-2026-07-31`. Both use
...
rustup toolchain install nightly-2026-07-31 --profile minimal --component llvm-tools-preview
...
3. Minimize a corpus with `tests/scripts/run-fuzz-target.sh cmin <target>` or a
   crash with `cargo +nightly-2026-07-31 fuzz tmin <target> <reproducer>`.
...
2. Reproduce with
   `cargo +nightly-2026-07-31 fuzz run <target> <reproducer>`.
3. Minimize with
   `cargo +nightly-2026-07-31 fuzz tmin <target> <reproducer>`.
