...
CI pins `nightly-2026-07-16` and `cargo-fuzz 0.13.2`. Install the same tools so
local reproduction does not silently use a different compiler or driver:

```sh
rustup toolchain install nightly-2026-07-16 --profile minimal --component llvm-tools-preview
...
3. Minimize a corpus with `tests/scripts/run-fuzz-target.sh cmin <target>` or a
   crash with `cargo +nightly-2026-07-16 fuzz tmin <target> <reproducer>`.
...
2. Reproduce with
   `cargo +nightly-2026-07-16 fuzz run <target> <reproducer>`.
3. Minimize with
   `cargo +nightly-2026-07-16 fuzz tmin <target> <reproducer>`.
