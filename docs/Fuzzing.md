...
| `proxy_protocol_tls` | PROXY v2 address framing and nested SSL-TLV metadata; malformed lengths, duplicate assertions, and invalid certificate metadata fail closed | Live sockets, trusted-peer admission, TLS handshakes, and certificate files |
...
AddressSanitizer and sustained profiles to `nightly-2026-08-30`. Both use
...
rustup toolchain install nightly-2026-08-30 --profile minimal --component llvm-tools-preview
...
3. Minimize a corpus with `tests/scripts/run-fuzz-target.sh cmin <target>` or a
   crash with `cargo +nightly-2026-08-30 fuzz tmin <target> <reproducer>`.
...
2. Reproduce with
   `cargo +nightly-2026-08-30 fuzz run <target> <reproducer>`.
3. Minimize with
   `cargo +nightly-2026-08-30 fuzz tmin <target> <reproducer>`.
