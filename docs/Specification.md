...
- `[access_log]` controls request-wide, WAF, and Admin API access-log sources and projects records to Elastic Common Schema (ECS), Open Cybersecurity Schema Framework (OCSF), or OTLP Logs with ECS-mapped bodies.
...
- OxiRule `emit_access_log` writes records with `scope = "waf"` into the shared access-log runtime, so stdout and OTLP delivery are selected under `[access_log.stdout]` and `[access_log.otlp]`.
