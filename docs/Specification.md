...
OxiBelt currently targets Rust 1.97 and uses `rustls` with the `aws-lc-rs` crypto provider. The default downstream TLS 1.3 key exchange group set enables `X25519MLKEM768`, `X25519`, `secp256r1`, and `secp384r1`; RFC 10024 `SecP256r1MLKEM768` is available only by explicit downstream-list or per-surface opt-in and is excluded from every default. Downstream TLS 1.3 and TLS 1.2 cipher suites can be restricted with `tls.1_3.ciphers` and `tls.1_2.groups`. Deployments can omit the hybrid group with `tls.1_3.key_exchange_groups` when cold-handshake CPU cost matters more than post-quantum hybrid negotiation.
...
Valid `Incremental: ?1` messages use [RFC 10036 incremental forwarding](Incremental.md). The signal is independent per direction; incompatible required buffering or inspection is refused rather than bypassed. Unmarked behavior and configuration defaults are unchanged.

Ordinary data-plane responses support built-in [RFC 9530 digest negotiation](https://www.rfc-editor.org/rfc/rfc9530.html) and `Unencoded-Digest` / `Want-Unencoded-Digest` from [draft-ietf-httpbis-unencoded-digest-05](https://datatracker.ietf.org/doc/html/draft-ietf-httpbis-unencoded-digest-05). Supported positive `Want-*` preferences select SHA-256 or SHA-512 independently per field. `Content-Digest` describes delivered content, `Repr-Digest` the complete selected encoded representation, and `Unencoded-Digest` its unencoded bytes. Existing applicable fields are preserved without verification; transformations remove fields whose covered bytes change. Generation never introduces buffering, decoding, or resource reads solely for digests, and preserves explicit route/WAF removals. See [HTTP digest negotiation](Configuration.md#http-digest-negotiation) for trailer requirements and omissions. The unencoded fields remain a pinned experimental draft contract; no configuration, rule grammar, or storage schema is added.
...
- Verified downstream client leaf certificates are available to OxiRule and certificate route matching on TCP TLS and HTTP/3/WebTransport. HTTP/3 certificate selectors remain fail-closed when metadata is absent or incomplete, and mTLS reserved-capacity eligibility remains TCP-only. OxiRule response and stream rules can inspect the serving upstream connection's bounded leaf metadata; this does not add pre-forward enforcement or change TLS acceptance. See [Peer certificate metadata](OxiRule.md#peer-certificate-metadata).
...
- HTTP/1.x CONNECT requires an authority-form target with an explicit valid port; HTTP/1.1 also requires exactly one matching valid `Host` field. The selected upstream connection and configured PROXY/TLS metadata egress are established before OxiBelt returns `200`. Every rejected CONNECT closes the downstream connection after its response, and successful CONNECT responses contain no `Content-Length` or `Transfer-Encoding`.
- WebTransport forwarding supports HTTP/2 and HTTP/3 extended CONNECT on either leg when the selected upstream has `webtransport = true`. HTTP/2 legs require TLS 1.3 and use `draft-ietf-webtrans-http2-15`; the existing HTTP/3 wire contract is unchanged. Configured upstream version selection is exact, with no transport fallback or CONNECT replay. See [WebTransport](WebTransport.md) for flow-control, admission, and reset behavior.
...
- WebRTC client-to-client media forwarding is supported through TURN listeners. Signaling HTTP requests can still be routed and inspected as ordinary HTTP traffic, while applications remain responsible for signaling, SDP exchange, and ICE candidate distribution. TURN `proxy_pool` transparently forwards UDP, TCP, and TLS control/data traffic to transport-matched external TURN pools; each `turns://` server has an independent bounded upstream TLS policy, and proxy UDP sessions have a finite per-listener cap. TURN `edge_relay` terminates source-bound RFC 8489 long-term authentication locally and allocates OxiBelt-managed UDP relay sockets or RFC 6062 TCP relay listeners for ICE candidates. RFC 8265 OpaqueString processing is applied to credential-key and USERHASH inputs; authenticated responses retain the selected integrity algorithm and identify OxiBelt with `SOFTWARE`; nonces bind the observed client tuple, realm, advertised password-algorithm policy, security-feature cookie, fresh random material, and issue time; active allocations retain one authenticated username. Method attributes after the first message-integrity attribute are ignored, so unsigned tail attributes cannot change allocation semantics. TCP allocations require a TCP/TLS control connection, enforce permissions for incoming peers, bound and expire pending data connections, and switch a separate authenticated `CONNECTION-BIND` connection to raw relay I/O. Edge relay supports IPv4 and IPv6 relay families, including clients behind IPv4 NAT and IPv6 NAT/NAT66 when the listener is configured with the matching relay family and public address. TURN `REQUESTED-ADDRESS-FAMILY` selects one configured family, `ADDITIONAL-ADDRESS-FAMILY = IPv6` can allocate both IPv4 and IPv6 relay sockets, and missing family requests default to IPv4. Edge relay enforces peer permissions before forwarding peer-to-client traffic, returns TURN capacity errors instead of failing listeners, clamps allocation lifetimes, and denies private, loopback, link-local, unspecified, multicast, broadcast, carrier-grade NAT/shared, protocol-assignment, benchmark, current-network, and reserved peer addresses by default. IPv4 peers must use the IPv4 relay family; IPv4-mapped IPv6 peer addresses are rejected. TURN `edge_relay` TCP/TLS outbound queues are bounded per downstream connection by `stream_outbound_queue_capacity`; the default is `32`, `"auto"` resolves conservatively from available parallelism with a `32..=64` clamp, explicit values are limited to `1..=256`, and full queues fail closed. Compatible snapshot reloads preserve unchanged TURN listener allocation state and stable upstream-pool runtime state while refreshing downstream TURN TLS configuration.
...
- Direct upstreams, static pool members, and discovery templates may opt into a certificate-root-relative upstream client certificate chain and matching private key. The identity is presented only when the authenticated upstream requests it, is inherited by forwarding, health checks, and diagnostics, and disables outbound TLS resumption and persistent client-config caching without weakening server certificate, hostname/SNI, SAN, ECH, or revocation verification.
...
- A terminal `actions.direct_response` error response.
...
Exact-uppercase HTTP QUERY follows the [QUERY contract](Query.md), including mandatory Content-Type validation, body-aware opt-in caching, and safe/idempotent retry and early-data handling.

Response caching supports tag extraction from configured response headers, exact/prefix/tag purges through the admin API, signed purge authentication, cache key explain, batch warming, collapsed forwarding with bounded follower waits, disk index recovery, admission filtering by status/MIME/body size/frequency, tenant partition keys, Surrogate-Control metadata, stale-if-error controls by error class or status, Vary explosion guards, fresh conditional `304` hits, `Age` on cached hits, single and multipart cached byte ranges, local disk streaming fills for large known-length objects, external L3 lookup/fill/revalidation/purge coordination, and background refresh for eligible stale-while-revalidate GET/HEAD and opt-in QUERY responses. RFC 9875 cache groups are enabled by default: response memberships and invalidations are bounded Structured Fields strings, scoped by canonical origin, policy, and partition, and persisted as bounded authority state. Non-safe 2xx/3xx final responses invalidate their exact target; other non-safe final responses require explicit valid group invalidations, while GET, HEAD, OPTIONS, TRACE, and QUERY do not trigger this mechanism. Direct membership selection makes one hop only. Group-aware L2 authority updates are atomic, L3 participation requires its negotiated capability, and recovery fences reuse before replacing the whole policy generation. The top-level `cache.no_vary_search` switch enables current draft-09 automatic No-Vary-Search handling for GET/HEAD/QUERY across all tiers: exact lookup is first, equivalent lookup and storage use bounded indexes, and both received and effective query representations must be covered. Explicit `{query:name}` tokens and raw partitions remain authoritative; response-WAF routes remain exact-only, and a legacy L3 handler without the optional capability remains exact-only. Equivalent aliases are invalidated conservatively by policy and origin path. Setting the switch to `false` disables alias lookup/storage while retaining forwarding and invalidation. Cache-enabled routes emit bounded `X-OxiBelt-Cache` and `X-OxiBelt-Cache-Reason` status headers and strip origin-supplied values for those names before caching or forwarding. Cache keys include scheme and host by default; production configurations should keep host and trusted variation dimensions in the key, avoid credential-bearing requests, and prefer query allowlists when only specific query parameters affect representation selection.
...
- `[admin]` exposes authenticated operations APIs such as OpenAPI metadata, cache purge, upstream-pool and stream-pool runtime control, dynamic policy automation, IPM management, config validation/load/rollback, explicit file sync, downstream TLS reload, and lifecycle drain/undrain on a dedicated listener. Plaintext Admin stays HTTP/1.1; Admin TLS always offers HTTP/1.1 and HTTP/2. Plaintext admin traffic is loopback-allowlisted by default; non-loopback admin traffic uses TLS unless the operator explicitly configures a plaintext source allowlist. IPM (Identity Permission Management) authorizes Admin operations and opt-in data-plane routes with `Action`, `Resource`, and `Condition` policy statements; explicit deny wins, matching allow permits, and the default is deny. Opt-in `[admin.workload_identity]` applies to Admin TCP TLS, HTTP/2, and HTTP/3: a chain-verified mTLS certificate's exact SPIFFE/URI/DNS SAN maps to one effective IPM principal, and any bearer or break-glass credential must resolve to that same principal. Its optional bearer mode permits the mapped certificate alone. TLS 1.3 HTTP/2 and enabled HTTP/3 operation-event WebTransport share authentication, authorization, audit, session-count, history, heartbeat, and terminal-event semantics; HTTP/2 rejects a supplied cross-origin `Origin`, accepts no app streams, and discards datagrams. The legacy Admin RBAC `roles`, `permissions`, and `deny_permissions` model is rejected. Full hot reload starts, stops, or rebinds this listener when admin listener settings change.
...
- `oxibelt-gateway-controller` is an optional Kubernetes control-plane binary. It watches Gateway API v1 `GatewayClass`, `Gateway`, `HTTPRoute`, `GRPCRoute`, `TLSRoute`, `TCPRoute`, `UDPRoute`, `BackendTLSPolicy`, `ReferenceGrant`, `Service`, and exact referenced public-CA ConfigMaps; when configured, it GETs only operator-allowlisted upstream-client Secret names and uses exact-name field-selected watches to trigger reconciliation, with the periodic reconcile loop retained as a safety fallback. It renders deterministic controller-owned TOML and public CA assets containing HTTP/gRPC routes and pools, Gateway HTTP external auth, bounded route actions, passthrough SNI rules, and raw TCP/UDP listeners and pools; validates the full artifact; and publishes a content-addressed immutable ConfigMap. TCPRoute and UDPRoute support one rule, protocol-compatible weighted core Service backends, exact parent/listener and ReferenceGrant enforcement, and oldest-creation-time winner selection without invalid-winner fallback. BackendTLSPolicy supports one same-namespace Service target with a required hostname, System trust or bounded ConfigMap `ca.crt` references, and exact DNS/URI SANs. A Gateway may independently select one core Secret through `spec.tls.backend.clientCertificateRef`; an operator allowlist, any required cross-namespace ReferenceGrant, bounded certificate/key validation, and an immutable target-namespace derived Secret are mandatory before upstream mTLS is rendered. Secret bytes are excluded from generated ConfigMaps, status, diagnostics, and digests. Unsupported TLS options, Secret CA refs, unauthorized or conflicting client identities, and pins are rejected rather than approximated. Multiple replicas use an exact-name `coordination.k8s.io/v1` Lease; only the current `(Lease UID, leaseTransitions, holderIdentity)` term may publish, patch, roll back, clean up, or mutate status. It only patches an opt-in named Deployment or DaemonSet and commits Gateway `Programmed` status after a fresh resource-version-guarded source snapshot, selected workload UID/generation/resource version, immutable digest, exact controller-owner Pod UID chain, and current leadership proof agree. Persisted workload annotations and immutable ConfigMaps make replacement-leader recovery independent of process memory. A newly invalid or unauthorized route/reference produces a sanitized revision rather than retaining stale authority; global snapshot, authorization, artifact, and final-validation failures roll back to the last committed ConfigMap. The base HTTP/HTTPS listener, downstream TLS, Admin/IPM, ACME/certificate lifecycle, `[sni_forward].enabled`, and public Service ports remain operator-owned. Raw TCP/UDP payloads bypass the HTTP WAF. Generated `UDPRoute` listeners are refused unless the controller is explicitly set to `shared_required` and every selected data-plane Pod has the matching shared-state configuration.
- Gateway client-identity invalidation is fail closed at the original HTTP/gRPC match boundary. Every completely reconstructable affected match becomes an empty terminal `503` route with the same generated name, hostname intersection, path, method, exact header/query or gRPC match, priority, and order, but no upstream, identity, filter, mirror, external-auth, route-policy, or WAF effect. Any unrelated translation error preserves the last good rollout. Tombstones require exact data-plane compatibility, remove managed identity mounts, and recover to normal forwarding only after the identity validates again.
