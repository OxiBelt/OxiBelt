...
- `[access_log]` controls request-wide, WAF, and Admin API access-log sources and projects records to Open Cybersecurity Schema Framework (OCSF) JSON on stdout. Elastic Common Schema and OpenTelemetry access-log export are reserved for later implementation and fail configuration loading if selected now.
...
- OxiRule `emit_access_log` writes records with `scope = "waf"` into the shared access-log runtime, and `[access_log.stdout]` emits OCSF HTTP Activity JSON for those records.
