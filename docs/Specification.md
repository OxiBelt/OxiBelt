...
- `[admin.mutations]` controls replay protection for configuration load/rollback, file sync, downstream TLS key reload, submitted typed secret-reference update, IPM writes, and break-glass activation/revocation. Active modes require PostgreSQL and enforcing durable audit. Secret-reference activation is currently unavailable and returns `409` without changing state. The ledger and audit records retain only bounded redacted results. Credential create/rotate returns a token only on first execution; an exact replay cannot re-emit it and reports `token_recoverable = false` in the reduced safe result.
...
- Admin audit events are structured first-class Admin events. `[admin.audit.store]` optionally writes them to PostgreSQL for `GET /admin/v1/audit`; `[admin.audit.export]` can route them through the Access Log Admin source for OCSF/ECS stdout or OTLP exports. Export sinks are best-effort observability integrations and are not query stores. `mode = "enforcing"` with the PostgreSQL store preserves fail-closed Admin mutation admission when durable audit reservation is unavailable. When workload binding is active, the audit records the certificate fingerprint, workload identity and principal, credential identity and principal, and fixed authentication reason without retaining raw certificate bytes.
...
- `POST /admin/v1/config/secret-references/update`: validates one allowlisted environment/file reference without accepting the secret value, then returns `409` because atomic runtime activation is unavailable.
