...
- `[access_log]` controls request-wide, WAF, and Admin API access-log sources and projects records to Open Cybersecurity Schema Framework (OCSF) or Elastic Common Schema (ECS) JSON on stdout. OpenTelemetry access-log export is reserved for later implementation and fails configuration loading if selected now.
...
- OxiRule `emit_access_log` writes records with `scope = "waf"` into the shared access-log runtime, and `[access_log.stdout]` emits OCSF HTTP Activity JSON or ECS HTTP/security JSON for those records.
