...
| Are request queues or upstream circuits limiting traffic? | `oxibelt_circuit_breaker_active`, `oxibelt_circuit_breaker_queued`, `oxibelt_circuit_breaker_rejections_total`, `oxibelt_circuit_breaker_state`, `oxibelt_circuit_breaker_transitions_total` | Alert on sustained queued work, rising `queue_timeout`/`retry_budget` rejections, or a route/pool `state="open"`. Scope labels are configuration-derived and capped; they never include paths, host headers, client identities, origins, or raw errors. |
