...
| Are certificates healthy? | `GET /admin/v1/tls/downstream`, `GET /admin/v1/tls/upstream`, `oxibelt_tls_ocsp_*`, `oxibelt_tls_crlite_*`, `oxibelt_tls_ct_*`, `oxibelt_tls_upstream_*`, TLS session storage metrics | Certificate inventory, including bounded SNI-selected downstream certificate entries, downstream OCSP/CRLite/CT state, and upstream revocation state stay on the authenticated Admin API. Public TLS health metrics use fixed series only and omit responder URLs, SNI, issuers, certificate fingerprints, serial numbers, SCTs, Log IDs, operators, and filter identifiers. |
...
Downstream CT follows the same audience split. Authenticated Admin TLS status, runtime snapshots, and support bundles contain bounded policy/list state plus per-certificate counts and compact error codes; they omit certificate identities, SNI, SCT signatures, Log IDs, operators, URLs, and cache paths. Public Prometheus output exposes only fixed-cardinality `oxibelt_tls_ct_checks_total{result="compliant|noncompliant|error"}`, `oxibelt_tls_ct_sct_verification_total{result="valid|invalid"}`, `oxibelt_tls_ct_enabled`, `oxibelt_tls_ct_noncompliant_certificates`, `oxibelt_tls_ct_log_list_age_seconds`, refresh success/error counters, and handshake rejection count. Log-list fetch and SCT verification run outside the handshake path.
