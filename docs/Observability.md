...
| Is shared state saturated? | `oxibelt_shared_state_queue_duration_ms`, `oxibelt_shared_state_operation_duration_ms`, `oxibelt_shared_state_queued_operations`, `oxibelt_shared_state_in_flight_operations`, `oxibelt_shared_state_operations_total`, `oxibelt_shared_state_enumeration_total`, `oxibelt_shared_state_deferred_cleanup_dropped_total`, `oxibelt_shared_state_pool_connections`, `oxibelt_shared_state_pool_waiters`, `oxibelt_shared_state_pool_max_connections`, `oxibelt_shared_state_pool_circuit_state`, `oxibelt_shared_state_pool_acquisitions_total`, `oxibelt_shared_state_pool_connection_events_total` | Alert on sustained queue growth, pool wait/create timeouts, a nonzero enumeration `cap_exhausted` event, or a non-closed Redis reconnect circuit before security-sensitive operations fail. Enumeration metrics use only configured backend/kind and fixed scope/event labels (`pages`, `scanned_keys`, `returned_keys`, `cap_exhausted`); no labels contain backend keys, request identity, tokens, URLs, or raw errors. |
