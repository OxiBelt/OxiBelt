...
| Are certificates healthy? | `GET /admin/v1/tls/downstream`, `GET /admin/v1/tls/upstream`, `oxibelt_tls_ocsp_*`, `oxibelt_tls_crlite_*`, `oxibelt_tls_upstream_*`, TLS session storage metrics | Certificate inventory, downstream OCSP/CRLite state, and upstream revocation state stay on the authenticated Admin API. Public TLS revocation metrics use fixed series only and omit responder URLs, SNI, issuers, certificate fingerprints, serial numbers, and filter identifiers. |
