...
| --- | --- | --- |
| Is the proxy up? | `/ready`, `/live`, `oxibelt_requests_total`, `oxibelt_responses_total` | Readiness returns `503 draining` while lifecycle drain is active. Health responses also include `X-OxiBelt-Backend-Status: healthy` or `degraded` without changing the status or response body. |
