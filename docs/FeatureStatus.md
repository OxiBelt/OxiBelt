...
| `helm-data-plane` | `experimental` | Deploy | The Helm chart under `deploy/helm/oxibelt` installs the OxiBelt data plane as a Deployment or DaemonSet with Service exposure, config and TLS mounts, probes, pod security defaults, metrics service wiring, PDB, and optional HPA. |
| `helm-gateway-controller` | `experimental` | Deploy | The Helm chart under `deploy/helm/oxibelt-gateway-controller` installs the controller deployment, service account, RBAC, Admin token secret reference, status-service and backend-resolution options, health probes, pod security defaults, and examples. |
