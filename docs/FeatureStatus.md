...
| `helm-data-plane` | `experimental` | Deploy | The Helm chart under `deploy/helm/oxibelt` installs the OxiBelt data plane as a Deployment or DaemonSet with immutable content-addressed base ConfigMaps, safe rollout defaults, read-only mounts, probes, pod security defaults, metrics service wiring, PDB, optional HPA, and an opt-in Kubernetes immutable rollout mode. |
