...
| `helm-data-plane` | `experimental` | Deploy | The Helm chart under `deploy/helm/oxibelt` installs the OxiBelt data plane as a Deployment or DaemonSet with immutable content-addressed base ConfigMaps, safe rollout defaults, read-only mounts, probes, pod security defaults, metrics service wiring, PDB, optional HPA, an opt-in Kubernetes immutable rollout mode, and validated TLS 1.3/mTLS Admin listener values. |
