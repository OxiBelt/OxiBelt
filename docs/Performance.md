...
The plain reverse-proxy fast path and plaintext static sendfile path are allowed to stay active for low-cost response metadata work such as configured security response headers and request-wide system access logs. Routes with `compression = "off"` can also use the plain reverse-proxy fast path even when global compression is enabled. For downstream H2/H3 `GET` and `HEAD` requests without framing headers, the fast path may perform one cooperative-yield EOF probe before constructing the upstream request body; it only sends an empty upstream body after observing EOF, and it keeps DATA, trailers, errors, delayed bodies, and framed requests on the bounded streaming path. Body-transforming compression, WAF HTTP body compression transform, cache lookup/fill, WAF inspection that needs body bytes, dynamic policy, rate limiting, upgrades, CONNECT, upstream pools, upstream HTTP/3, PROXY protocol egress, and buffering remain on the general proxy path so HTTP and security semantics stay unchanged. Basic metrics expose `oxibelt_http_fast_path_decisions_total{path="plain_proxy",protocol="h1|h2|h3|other",outcome="hit|miss",reason="..."}` with fixed low-cardinality labels. The direct upstream HTTP/1.1 transport exposes `oxibelt_http_fast_path_transports_total{transport="direct_h1",protocol="h1|h2|h3|other",outcome="hit|miss",reason="..."}` and is limited to empty-body downstream HTTP/1.1 or proven-empty downstream HTTP/2 and HTTP/3 `GET` and `HEAD` requests that target a direct plaintext HTTP/1.1 upstream without PROXY protocol egress. The guarded direct upstream HTTP/2 transport uses the same direct-selection and empty-body preconditions for HTTP/2 or H2C upstreams and reports fixed-label diagnostics with `transport="direct_h2"`. Benchmark reverse-proxy runs snapshot the plain-proxy, direct-H1, and direct-H2 metrics around the gated OxiBelt rows, store deltas under `results.json` as `fast_path.plain_proxy.*` and `fast_path.transport.direct_h1|direct_h2.*`, and require expected hit rates to meet `OXIBELT_PERF_H1_FAST_PATH_MIN_HIT_RATE`, which defaults to `0.99`. The common `oxibelt-h1-keepalive`, `oxibelt-h2`, and `oxibelt-h3` rows gate direct-H1 transport evidence; the split `oxibelt-h2-upstream-h2c` and `oxibelt-h2-upstream-h2` rows gate direct-H2 transport evidence. These rows also store direct-H1 pool event deltas under `fast_path.pool.direct_h1`; comparison schema 20 aggregates those event counts as diagnostics, not gates. Benchmark reverse-proxy runs also include an OxiBelt-only `oxibelt-h2-adaptive-window` row that keeps the same upstream HTTP/1.1 fixture but restores adaptive H2 windows as diagnostic evidence against the fixed-tuning baseline. This diagnostic row does not change the default product `proxy.http2` values or regression gates.
