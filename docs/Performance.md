...
- `performance-comparison.json`: schema `19`, with aggregate rows, baseline selection metadata, sample quality, evidence quorum, per-shard distributions, H1/H2/H3 fast-path and expected direct-transport hit-rate evidence, separate `external_benchmarks` and `profiling` rows, and regression gate decisions.
