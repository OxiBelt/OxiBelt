...
OpenResty rows are retained as comparator diagnostics. If the OpenResty fixture
produces a malformed or zero-request probe row, the harness records that row as
a skipped diagnostic comparator result instead of failing the iteration; OxiBelt,
nginx, and Caddy rows remain strict primary evidence.
...
- `performance-comparison.json`: schema `20`, with aggregate rows, baseline selection metadata, sample quality, evidence quorum, per-shard distributions, H1/H2/H3 fast-path and expected direct-transport hit-rate evidence, separate `external_benchmarks` and `profiling` rows, and regression gate decisions.
