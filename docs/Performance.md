...
- `performance-comparison.json`: schema `18`, with aggregate rows, baseline selection metadata, sample quality, evidence quorum, per-shard distributions, H1/H2/H3 fast-path and direct-H1 transport hit-rate evidence, separate `external_benchmarks` and `profiling` rows, and regression gate decisions.
