...
- `performance-comparison.json`: schema `17`, with aggregate rows, baseline selection metadata, sample quality, evidence quorum, per-shard distributions, H1/H2 fast-path and direct-H1 transport hit-rate evidence, separate `external_benchmarks` and `profiling` rows, and regression gate decisions.
