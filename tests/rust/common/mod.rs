...
#[allow(dead_code)]
pub fn create_ca_signed_client_cert(
  dir: &Path,
  common_name: &str,
  ca_cert_path: &Path,
  ca_key_path: &Path,
  ...
  let id = next_test_id();
  let key_path = dir.join(format!("client-{id}.key"));
  let cert_path = dir.join(format!("client-{id}.pem"));
  let csr_path = dir.join(format!("client-{id}.csr"));
  let config_path = dir.join(format!("client-{id}.cnf"));
