...
pub(super) fn docker_cases() -> Vec<DockerCase> {
  vec![
    ...
    docker_case(
      ...
      "verified downstream client certificate leaves are forwarded safely over HTTP/1.1, HTTP/2, HTTP/3, WebSocket, and WebTransport",
      ExpectStart::Success,
      Needs {
        https_upstream: true,
        h2_upstream: true,
    ...
    docker_case(
      ...
      "required downstream mTLS forwards RFC 9440 Client-Cert while every upstream authenticates OxiBelt with an independent client identity",
      ExpectStart::Success,
      Needs {
        https_upstream: true,
        h2_upstream: true,
