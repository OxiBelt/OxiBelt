...
fn performance_probe_image_override_skips_local_probe_build() {
  ...
  assert!(
    script
      .contains("perf_probe_image=\"${OXIBELT_PERF_PROBE_IMAGE:-oxibelt/perf-probe:${run_id}}\""),
...
fn external_benchmark_failures_warn_by_default_and_can_fail_closed() {
  ...
  assert!(
    String::from_utf8_lossy(&warn.output.stderr)
      .contains("External benchmark validation warning: synthetic external benchmark failure"),
...
fn external_h2load_h3_zero_flush_suppresses_cross_comparator_diagnostics() {
  ...
  assert!(
    !String::from_utf8_lossy(&run.output.stderr).contains("External benchmark validation warning"),
...
fn diagnostic_profile_layer_keeps_primary_results_separate() {
  ...
  assert!(
    script.contains("append_profile_result()")
      ...
      && script.contains("jq -s '.' \"${profile_results_jsonl}\" >\"${profile_results_json}\""),
...
fn local_performance_probe_build_retries_base_pulls_and_build() {
  ...
  assert!(
    script.contains("for base_image in rust:1.98.1-trixie@sha256:737ba17e6a2ffe14475b59861cd69f3d7152c29c75140bdbf6750befcfda7e6c debian:trixie-slim@sha256:d7e12182ce18b85b93007c1dedf31f2d29e01ccf3182cc4017c709b6259bc132; do")
...
fn local_external_benchmark_build_retries_base_pulls_and_build() {
  ...
  assert!(
    script.contains("for base_image in rust:1.98.1-trixie@sha256:737ba17e6a2ffe14475b59861cd69f3d7152c29c75140bdbf6750befcfda7e6c debian:trixie@sha256:f324c7ff54321e8d9c588493a20244965938ce0aa50bbd1022d38010e9ffc4b1 debian:trixie-slim@sha256:d7e12182ce18b85b93007c1dedf31f2d29e01ccf3182cc4017c709b6259bc132; do")
      && script.contains("retry_command 3 docker pull \"${base_image}\"")
      && script.contains("retry_command 3 docker build")
...
fn diagnostic_perf_profile_fails_when_oxibelt_pid_is_missing() {
  ...
  assert!(
    missing_pid
      .events
      .contains("FAIL profiling requested for oxibelt-h2, but OxiBelt host PID was not available")
  );
}
...
fn handshake_resumption_diagnostic_runs_without_replacing_cold_row() {
  ...
  assert!(
    script.contains("run_handshake_with_options \"$1\" \"$2\" \"$3\" worker 25 diagnostic none"),
...
fn tls_resumption_mode_handshake_rows_run_as_fresh_oxibelt_only_smoke_rows() {
  ...
  assert!(
    script.contains("run_handshake_with_options \"$1\" \"$2\" \"$3\" fresh 0 strict tls-storage"),
