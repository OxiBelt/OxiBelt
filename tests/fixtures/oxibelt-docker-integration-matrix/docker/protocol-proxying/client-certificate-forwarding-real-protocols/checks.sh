run_case_checks() {
  ...
  assert_forwarded_header() {
    local candidate="$1"
    local expected="$2"
    if ! jq -e --arg expected "${expected}" '.body | fromjson | .headers["x-verified-client-cert"] == $expected and .headers["x_verified_client_cert"] == null' <<<"${candidate}" >/dev/null; then
  ...
  assert_h1_forwarded_header() {
    local candidate="$1"
    local expected="$2"
    if ! jq -e --arg expected "${expected}" '.response_base64 | @base64d | split("\r\n\r\n")[1] | fromjson | .headers["x-verified-client-cert"] == $expected and .headers["x_verified_client_cert"] == null' <<<"${candidate}" >/dev/null; then
  ...
  for protocol in h1 h2 h3; do
    ...
    if [[ "${protocol}" == "h1" ]]; then
      response_urlpem="$(protocol_probe_http1_client_with_client_identity "forward-h1-urlpem.example.test" "/certificate/${protocol}" "x_verified_client_cert: attacker")"
      response_rfc9440="$(protocol_probe_http1_client_with_client_identity "forward-h1-rfc9440.example.test" "/certificate/${protocol}" "x_verified_client_cert: attacker")"
    else
      response_urlpem="$(protocol_probe_client_with_client_identity "${protocol}" "forward-${protocol}-urlpem.example.test" "/certificate/${protocol}" 200 --header 'x_verified_client_cert: attacker')"
      response_rfc9440="$(protocol_probe_client_with_client_identity "${protocol}" "forward-${protocol}-rfc9440.example.test" "/certificate/${protocol}" 200 --header 'x_verified_client_cert: attacker')"
    fi
    if [[ "${protocol}" == "h1" ]]; then
  ...
  response="$(protocol_probe_client "h2" "forward-h2-urlpem.example.test" "/certificate/absent" 200 --header 'x_verified_client_cert: attacker')"
  assert_body_jq "${response}" '.headers["x-verified-client-cert"] == null and .headers["x_verified_client_cert"] == null'

  response="$(protocol_probe_client_with_explicit_identity h2 "forward-h2-urlpem.example.test" "/certificate/spoof" 200 "${client_tls_dir}/client.pem" "${client_tls_dir}/client.key" --header 'x-verified-client-cert: attacker-one' --header 'X-Verified-Client-Cert: attacker-two' --header 'x_verified_client_cert: attacker-alias')"
  assert_forwarded_header "${response}" "${urlpem}"

  websocket="$(protocol_probe_websocket_client_with_client_identity "forward-ws.example.test" "/ws/forward" "verified-websocket" --header 'x_verified_client_cert: attacker')"
  assert_response_jq "${websocket}" '.status == 101 and .upgraded == true and .echoed_bytes == 18'
  webtransport="$(protocol_probe_webtransport_multiplex_with_client_identity "forward-wt.example.test" "/wt/forward" --header 'x_verified_client_cert: attacker')"
  ...
  absent_first="$(protocol_probe_client h2 "forward-cache.example.test" "/certificate/cache?cache_control=public%2Cmax-age%3D60" 200 --header 'x_verified_client_cert: attacker-one')"
  absent_hit="$(protocol_probe_client h2 "forward-cache.example.test" "/certificate/cache?cache_control=public%2Cmax-age%3D60" 200 --header 'x_verified_client_cert: attacker-two')"
  ...
  assert_body_jq "${absent_first}" '.headers["x-verified-client-cert"] == null and .headers["x_verified_client_cert"] == null'
  assert_body_jq "${absent_hit}" '.headers["x-verified-client-cert"] == null and .headers["x_verified_client_cert"] == null'
