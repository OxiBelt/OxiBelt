...
run_case_checks() {
  local gate_id="retry-storm" request_path burst_file burst_pid gate_status metrics
  ...
  for _ in $(seq 1 100); do
    ...
    active_retry="$(jq -r '.body' <<<"${metrics}" | awk '
      index($1, "oxibelt_circuit_breaker_active{") == 1 && index($0, "scope_kind=\"global\"") && index($0, "scope=\"global\"") && index($0, "kind=\"retry\"") { value = $NF }
      END { print value }
    ')"
    ...
    if [[ "${active_retry}" == "1" && "${observed_budget}" == "15" ]]; then
      break
    fi
    sleep 0.1
  done
  if [[ "${active_retry}" != "1" || "${observed_budget}" != "15" ]]; then
    fail_with_diagnostics "retry storm did not converge to one active retry and fifteen budget rejections"
  fi
  live="$(plain_client_request_on_port 9091 "ops.test" "/live" 200)"
  ...
  active_retry="$(jq -r '.body' <<<"${metrics}" | awk '
    ...
    END { print value }
  ')"
  queued_retry="$(jq -r '.body' <<<"${metrics}" | awk '
    index($1, "oxibelt_circuit_breaker_queued{") == 1 && index($0, "scope_kind=\"global\"") && index($0, "scope=\"global\"") && index($0, "kind=\"retry\"") { value = $NF }
    END { print value }
  ')"
