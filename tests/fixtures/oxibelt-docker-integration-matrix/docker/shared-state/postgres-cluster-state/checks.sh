...
assert_shared_query_cache_cleanup() {
  ...
  for attempt in $(seq 1 50); do
    index_count="$(postgres_query "SELECT count(*) FROM oxibelt_shared_cache_query_targets WHERE namespace_key = 'matrix-shared';")"
    [[ "${index_count}" == "1" ]] && break
    sleep 0.1
  done
  if [[ "${index_count}" != "1" ]]; then
    fail_with_diagnostics "expected PostgreSQL QUERY cleanup to retain only the current indexed generation, got ${index_count}"
  fi
}
