...
container_name="oxibelt-admin-mutation-postgres-${run_token}"
...
if [[ "${REMOTE_CONTAINERS:-}" == "true" ]]; then
  # This devcontainer can reach sibling rootless containers directly. Avoid
  # opening the test database on a host interface merely to cross namespaces.
  docker_publish_args=()
fi
...
export OXIBELT_REQUIRE_MUTATION_POSTGRES_TESTS=1
export OXIBELT_TEST_MUTATION_POSTGRES_URL="postgres://oxibelt:${postgres_password}@${postgres_connect_host}:${host_port}/oxibelt"
