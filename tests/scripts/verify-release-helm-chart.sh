...
`rebuild` requires Helm v4.3.0, an approved `helm_chart_release.ts` helper,
and exact source inputs. `consume` permits Helm v3.22.0 or v4.3.0 and never
...
if [[ "${mode}" == rebuild ]]; then
  [[ "${helm_version}" =~ ^v4\.3\.0(\+[0-9A-Za-z.-]+)?$ ]] || { echo "byte rebuild requires Helm v4.3.0, found ${helm_version}" >&2; exit 1; }
  [[ "${release_ref}" == "refs/tags/${version}" && "${revision}" =~ ^[0-9a-f]{40}$ && -n "${workspace_path}" ]] || { usage; exit 2; }
else
  [[ "${helm_version}" =~ ^v(3\.22\.0|4\.3\.0)(\+[0-9A-Za-z.-]+)?$ ]] || { echo "consumption verification requires Helm 3.22.0 or 4.3.0, found ${helm_version}" >&2; exit 1; }
fi
