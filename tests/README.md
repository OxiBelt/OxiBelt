...
- `scripts/run-browser-webdriver-check.sh`: starts a mock upstream and validates that Chromium or Firefox WebDriver can reach OxiBelt through either a local release binary or an `OXIBELT_DOCKER_IMAGE` container. Pass a scenario name (`basic-navigation`, `waf-request`, `waf-response`, `person-proof`, `hot-reload`, or `webrtc-turn`) to run the corresponding browser-level check. The WebRTC scenario establishes relay-only IPv4 data channels over IPv4 and IPv6 TURN control endpoints using UDP, TCP, and TLS; explicit IPv6 relay allocation remains covered by the coturn interoperability scenario.
