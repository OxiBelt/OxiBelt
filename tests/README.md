...
- `scripts/lib/verified-download.sh`: downloads pinned CI inputs into temporary staging, verifies SHA-256, and atomically publishes the verified file. HTTP and transport failures receive up to eight retries with exponential backoff, a 10-second connection timeout, a 60-second transfer timeout, and a 300-second retry window; a final transfer may finish after that window. Digest mismatches fail immediately. Executable recovery tests run within `ci_workflow_integrity`.
