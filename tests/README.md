...
- `scripts/run-proxy-performance.sh`: runs Docker-network performance smoke, benchmark, or soak profiles after integration coverage. It starts OxiBelt, nginx, Caddy, and `docker/perf_probe/` in one Docker network, fails closed for mandatory OxiBelt/Caddy HTTP/3 probe failures, applies the OxiBelt H1/H2 baseline latency-floor gate, and writes `summary.md`, `results.json`, `docker-stats.jsonl`, logs, and generated configs.
