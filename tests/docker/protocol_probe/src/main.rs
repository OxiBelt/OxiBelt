...
fn usage() {
  ...
  eprintln!(
    "incremental-client also accepts --expect-status <200|204>, --response-mode <duplex|fixed-length-200>, --resumable-relay, --resumable-relay-only, --expect-proxy-status <value>, and --expect-cache-status <value>"
  );
}
...
pub(crate) fn downstream_client_config(
  path: &Path,
  alpn: &[u8],
  tls_version: Option<DownstreamTlsVersion>,
...
pub(crate) fn upstream_tls_server_config(
  cert: &str,
  key: &str,
  client_ca: Option<&str>,
  tls13_only: bool,
