...
pub(super) async fn admin_upstream_pools_response(
  ...
  match (
    ...
    (Some(pool_name), None, None, None) => {
      if !authorization.is_allowed("upstream-pool:Get", pool_name) {
        return Some(text_response(StatusCode::FORBIDDEN, "forbidden"));
      }
      if *method != ::http::Method::GET {
        return Some(text_response(
    ...
    (Some(pool_name), Some("servers"), None, None) => {
      ...
      let resource = admin_resource::upstream_pool_server(pool_name, &body.id);
    ...
    (Some(pool_name), Some("servers"), Some(server_id), None) => {
      ...
      let resource = admin_resource::upstream_pool_server(pool_name, server_id);
