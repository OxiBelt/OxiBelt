...
pub struct RecvStream {
    ...
    all_data_read: bool,
    reset: Option<VarInt>,
    reset_info: Option<proto::ResetInfo>,
}
...
impl RecvStream {
    ...
    pub async fn received_reset(&mut self) -> Result<Option<VarInt>, ResetError> {
        self.received_reset_info()
            .await
            .map(|info| info.map(|info| info.error_code))
    }

    /// Return reset metadata, including the peer's final size, when available.
    ///
    /// This remains available after a read surfaces the reset error.
    pub fn reset_info(&self) -> Option<proto::ResetInfo> {
        let mut conn = self.conn.state.lock("RecvStream::reset_info");
        conn.inner.recv_stream(self.stream).reset_info().or(self.reset_info)
    }

    /// Complete when the peer resets the stream and return its final-size metadata.
    pub async fn received_reset_info(&mut self) -> Result<Option<proto::ResetInfo>, ResetError> {
        poll_fn(|cx| {
            ...
            if let Some(info) = self.reset_info {
                let latest = conn.inner.recv_stream(self.stream).reset_info().unwrap_or(info);
                self.reset_info = Some(latest);
                return Poll::Ready(Ok(Some(latest)));
            }

            match conn.inner.recv_stream(self.stream).received_reset_info() {
                Err(_) => Poll::Ready(Ok(None)),
                Ok(Some(info)) => {
                    // A normal reset frees stream state; a partial reset leaves its prefix readable.
                    self.reset_info = Some(info);
                    conn.wake();
        ...
        T: FnMut(&mut Chunks) -> ReadStatus<U>,
    {
        ...
        let status = match self.reset {
            ...
            None => {
                ...
                self.reset_info = recv.reset_info().or(self.reset_info);
