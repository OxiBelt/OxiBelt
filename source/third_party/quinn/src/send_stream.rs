...
impl SendStream {
    ...
    /// Reset while guaranteeing delivery of the first `reliable_size` bytes.
    ///
    /// This requires the peer to advertise draft-09 RESET_STREAM_AT support.
    pub fn reset_at(&mut self, error_code: VarInt, reliable_size: u64) -> Result<(), proto::ResetAtError> {
        let mut conn = self.conn.state.lock("SendStream::reset_at");
        if self.is_0rtt && conn.check_0rtt().is_err() {
            return Err(proto::ResetAtError::ClosedStream);
        }
        conn.inner.send_stream(self.stream).reset_at(error_code, reliable_size)?;
        conn.wake();
        Ok(())
    }
