...
#[path = "sending_epoch.rs"]
mod sending_epoch;

use sending_epoch::SendingEpoch;
    ...
    pub(crate) fn into_traffic(
        ...
        (
            KeyScheduleTraffic {
                ...
                current_server_traffic_secret,
                current_exporter_secret,
                sending_epoch: SendingEpoch::default(),
            },
...
pub(crate) struct KeyScheduleTraffic {
    ...
    current_server_traffic_secret: OkmBlock,
    current_exporter_secret: OkmBlock,
    sending_epoch: SendingEpoch,
}

impl KeyScheduleTraffic {
    pub(crate) fn update_encrypter_and_notify(&mut self, common: &mut CommonState) {
        if !self.sending_epoch.advance() {
            // The request_update flag may be ignored when the sender has
            // reached its epoch limit. A later attempted local refresh closes
            // the connection before the current traffic key wears out.
            return;
        }
    ...
    pub(crate) fn request_key_update_and_update_encrypter(
        ...
        if !self.sending_epoch.advance() {
            ...
            return Err(Error::General("TLS 1.3 key update epoch exhausted".into()));
        }
        // A second update_requested is forbidden until a KeyUpdate arrives
        // from the peer. A one-sided refresh can use update_not_requested.
        if common.key_update_request_outstanding {
            ...
            common.key_update_request_outstanding = true;
        }
        let secret = self.next_application_traffic_secret(common.side);
        self.ks.set_encrypter(&secret, common);
        Ok(())
