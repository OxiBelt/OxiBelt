...
settings! {
    ...
    // draft-ietf-webtrans-http3-16, Sections 3.1 and 9.2.
...
impl Settings {
    pub fn decode<B: Buf>(buf: &mut B) -> Result<Self, SettingsError> {
        ...
        while data.has_remaining() {
            ...
            if !id.is_grease() {
                if id == Setting::WT_ENABLED && value.into_inner() > 1 {
                    return Err(SettingsError::InvalidValue);
                }
                if settings.0.insert(id, value).is_some() {
    ...
    /// Advertise draft 16 with bounded initial receive credit.
    pub fn enable_webtransport_draft16(&mut self, uni: u32, bidi: u32, data: u32) {
        self.insert(Setting::ENABLE_CONNECT_PROTOCOL, VarInt::from_u32(1));
        self.insert(Setting::ENABLE_DATAGRAM, VarInt::from_u32(1));
        self.insert(Setting::WT_ENABLED, VarInt::from_u32(1));
        self.insert(Setting::WT_INITIAL_MAX_STREAMS_UNI, VarInt::from_u32(uni));
        self.insert(Setting::WT_INITIAL_MAX_STREAMS_BIDI, VarInt::from_u32(bidi));
        self.insert(Setting::WT_INITIAL_MAX_DATA, VarInt::from_u32(data));
    }

    /// True only if all mandatory draft 16 HTTP/3 settings are present.
    pub fn supports_webtransport_draft16(&self) -> bool {
        self.get(&Setting::WT_ENABLED).map(|v| v.into_inner()) == Some(1)
            && self.get(&Setting::ENABLE_CONNECT_PROTOCOL).map(|v| v.into_inner()) == Some(1)
            && self.get(&Setting::ENABLE_DATAGRAM).map(|v| v.into_inner()) == Some(1)
    }

    /// Initial stream and data credit granted by the peer for each draft 16 session.
    pub fn draft16_initial_credit(&self) -> (u64, u64, u64) {
        let value = |key| self.get(&key).map_or(0, |v| v.into_inner());
        (
            value(Setting::WT_INITIAL_MAX_STREAMS_UNI),
            value(Setting::WT_INITIAL_MAX_STREAMS_BIDI),
            value(Setting::WT_INITIAL_MAX_DATA),
        )
    }
...
mod tests {
    ...
    #[test]
    fn draft16_settings_wire_has_required_capabilities_and_zero_credit() {
        let mut settings = Settings::default();
        ...
        assert!(decoded.supports_webtransport_draft16());
        assert_eq!(decoded.draft16_initial_credit(), (0, 0, 0));
        assert_eq!(decoded.get(&Setting::ENABLE_CONNECT_PROTOCOL).map(|v| v.into_inner()), Some(1));
        assert_eq!(decoded.get(&Setting::ENABLE_DATAGRAM).map(|v| v.into_inner()), Some(1));
    }
    use std::io::Cursor;
