...
impl VarInt {
    ...
    /// Read a required QUIC varint and retain its actual encoded width.
    pub async fn read_with_width<S: AsyncRead + Unpin>(stream: &mut S) -> Result<(Self, usize), VarIntUnexpectedEnd> {
        let mut buf = [0u8; 8];
        stream.read_exact(&mut buf[..1]).await.map_err(|_| VarIntUnexpectedEnd)?;
        let width = 1usize << (buf[0] >> 6);
        stream.read_exact(&mut buf[1..width]).await.map_err(|_| VarIntUnexpectedEnd)?;
        Ok((Self::decode(&mut &buf[..width])?, width))
    }
