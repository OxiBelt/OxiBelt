...
mod tests {
    ...
    fn test_truncated_error_code() {
        ...
        let mut buf = data.as_slice();
        let result = Capsule::decode(&mut buf);
        assert!(matches!(result, Err(CapsuleError::InvalidLength)));
    }

    #[test]
    fn short_declared_close_length_fails_before_more_data_arrives() {
        for length in [0, 3] {
            let mut wire = Vec::new();
            VarInt::from_u64(0x2843).unwrap().encode(&mut wire);
            VarInt::from_u32(length).encode(&mut wire);
            wire.resize(wire.len() + length as usize, 0);
            assert!(matches!(Capsule::decode(&mut wire.as_slice()), Err(CapsuleError::InvalidLength)));
        }
    }
