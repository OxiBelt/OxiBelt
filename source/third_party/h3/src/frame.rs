        ...
        loop {
            ...
            return match self.decoder.decode(self.stream.buf_mut())? {
                ...
                frame @ Some(Frame::WebTransportStream(_)) => {
                    ...
                    self.stream.set_webtransport_header_size(self.decoder.consumed);
                    Poll::Ready(Ok(frame))
                }
...
mod tests {
    ...
    #[test]
    fn webtransport_header_counts_nonminimal_varint_width() {
        // Type 0x41 and session ID 1 are both legal two-byte QUIC varints.
        let mut bytes = BufList::from(Bytes::from_static(&[0x40, 0x41, 0x40, 0x01]));
        let mut decoder = FrameDecoder::default();
        assert!(matches!(decoder.decode(&mut bytes), Ok(Some(Frame::WebTransportStream(_)))));
