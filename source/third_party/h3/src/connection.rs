...
const MAX_UNASSOCIATED_UNI_RESETS: usize = 128;

fn record_unassociated_uni_reset(
    events: &mut Vec<(quic::StreamId, u64)>,
    id: quic::StreamId,
    error_code: u64,
) {
    if events.len() < MAX_UNASSOCIATED_UNI_RESETS {
        events.push((id, error_code));
    }
}
    ...
    B: Buf,
{
    ...
    /// Resets of unidirectional streams whose WebTransport session ID was not available.
    /// A caller must associate these only when its own session policy permits it.
    pub wt_unassociated_uni_resets: Vec<(quic::StreamId, u64)>,
}
    ...
    B: Buf,
{
    fn default() -> Self {
        Self {
            ...
            wt_unassociated_uni_resets: Default::default(),
        }
    }
}
    ...
    pub fn poll_accept_recv(&mut self, cx: &mut Context<'_>) -> Result<(), ConnectionError> {
        ...
        for stream in self.pending_recv_streams.iter_mut().filter(|s| s.is_some()) {
            let resolved = match stream.as_mut().expect("this cannot be None").poll_type(cx) {
                ...
                Poll::Ready(Err(stream::PollTypeError::EndOfStream { reset_code })) =>
                //= https://www.rfc-editor.org/rfc/rfc9114#section-6.2
                //# A receiver MUST tolerate unidirectional streams being
                //# closed or reset prior to the reception of the unidirectional stream
                //# header.
                {
                    // Keep a bounded transport-only hint for WebTransport. The stream type may
                    // be absent entirely, so the application must decide whether any session
                    // can safely claim this reset. HTTP/3 still tolerates the early reset.
                    if let Some(error_code) = reset_code {
                        let pending = stream.as_ref().expect("this cannot be None");
                        if pending.could_be_webtransport()
                            && (self.config.settings.enable_webtransport
                                || self.config.settings.enable_webtransport_h3)
                        {
                            record_unassociated_uni_reset(
                                &mut self.accepted_streams.wt_unassociated_uni_resets,
                                pending.recv_id(),
                                error_code,
                            );
                        }
...
mod unassociated_reset_tests {
    ...
    #[test]
    fn early_uni_reset_queue_is_bounded_and_reusable() {
        let mut events = Vec::new();
        for index in 0..=MAX_UNASSOCIATED_UNI_RESETS {
            let id = quic::StreamId::try_from(2 + 4 * index as u64).unwrap();
            record_unassociated_uni_reset(&mut events, id, 0x52e4a40fa8db);
        }
        assert_eq!(events.len(), MAX_UNASSOCIATED_UNI_RESETS);
        assert_eq!(
            events.last().unwrap().0.index(),
            MAX_UNASSOCIATED_UNI_RESETS as u64 - 1
        );

        events.pop();
        record_unassociated_uni_reset(
            &mut events,
            quic::StreamId::try_from(2 + 4 * MAX_UNASSOCIATED_UNI_RESETS as u64).unwrap(),
            7,
        );
        assert_eq!(events.len(), MAX_UNASSOCIATED_UNI_RESETS);
        assert_eq!(events.last().unwrap().1, 7);
    }
}
