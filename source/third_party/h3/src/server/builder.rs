...
impl Builder {
    ...
    /// Advertise support for WebTransport over HTTP/3 draft 16.
    pub fn enable_webtransport_h3(&mut self, value: bool) -> &mut Self {
        self.config.settings.enable_webtransport_h3 = value;
        self
    }

    /// Advertise initial draft 16 per-session receive credit.
    pub fn webtransport_h3_initial_credit(&mut self, uni: u64, bidi: u64, data: u64) -> &mut Self {
        self.config.settings.wt_initial_max_streams_uni = uni;
        self.config.settings.wt_initial_max_streams_bidi = bidi;
        self.config.settings.wt_initial_max_data = data;
        self
    }
