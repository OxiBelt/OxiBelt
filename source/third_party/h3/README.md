...
Get more information about this tool in the upstream contributing guide.
...
* Async only API
* QUIC transport abstraction via traits in the [`quic`](src/quic.rs) module
* Runtime independent (h3 does not spawn tasks and works with any runtime)
* Supported QUIC implementations to date are
  [Quinn](https://github.com/quinn-rs/quinn) (h3-quinn)
...
The upstream examples directory can help get started in two ways:
...
You can find a full server example in the upstream `examples/server.rs`.

### Client

You can find a full client example in the upstream `examples/client.rs`.
...
- `h3-quinn`: in the upstream repository.
- [`s2n-quic-h3`](https://github.com/aws/s2n-quic/tree/main/quic/s2n-quic-h3)
- [`h3-msquic-async`](https://github.com/masa-koz/msquic-async-rs/tree/main/h3-msquic-async)
