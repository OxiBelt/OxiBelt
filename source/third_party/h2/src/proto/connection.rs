...
pub(crate) struct Config {
  ...
  pub webtransport: crate::webtransport::SettingsHandle,
...
where
  ...
  B: Buf,
{
  pub(crate) fn webtransport_settings(&self) -> crate::webtransport::SettingsHandle {
    self.inner.settings.webtransport_settings()
  }
  pub fn new(codec: Codec<T, Prioritized<B>>, config: Config) -> Connection<T, P, B> {
    ...
    Connection {
      ...
      inner: ConnectionInner {
        ...
        settings: Settings::new(config.settings, config.webtransport),
        streams,
        span,
  ...
  pub(crate) fn set_webtransport_settings(
    &mut self,
    settings: crate::webtransport::Settings,
  ) -> Result<(), UserError> {
    let mut frame = frame::Settings::default();
    frame.set_webtransport_settings(settings);
        ...
        .recv_frame(ready!(Pin::new(&mut self.codec).poll_next(cx)?))?
      {
        ReceivedFrame::Settings(frame) => {
          self
            .inner
            .settings
            .recv_settings(frame, &mut self.codec, &mut self.inner.streams)?;
        }
      ...
      Err(Error::Io(kind, inner)) => {
        ...
        if self.streams.is_buffer_empty()
          ...
          && (self.streams.is_server()
            || self.error.as_ref().map(|f| f.reason() == Reason::NO_ERROR) == Some(true))
        {
  ...
  fn recv_frame(&mut self, frame: Option<Frame>) -> Result<ReceivedFrame, Error> {
    ...
    match frame {
      Some(Headers(frame)) => {
        ...
        // Capture the sending endpoint's acknowledged settings while the
        // HEADERS frame is processed. `poll_accept` and response futures can
        // subsequently drain later frames before exposing this message.
        let settings = crate::webtransport::ReceivedSettingsSnapshot::new(
          self.settings.webtransport_settings().local_acknowledged(),
        );
        self.streams.recv_headers(frame, settings)?;
      }
      Some(Data(frame)) => {
