...
impl Prioritize {
        ...
        B: Buf,
    {
        ...
        loop {
            ...
            match self.pop_frame(buffer, store, max_frame_len, counts, webtransport) {
                Some(frame) => {
                ...
                Some(mut stream) => {
                    ...
                    let frame = match stream.pending_send.pop_front(buffer) {
                        ...
                        Some(Frame::Headers(headers))
                            if stream.webtransport_outbound
                                && stream.webtransport_outbound_snapshot.is_none()
                                && !headers
                                    .pseudo()
                                    .status
                                    .is_some_and(|status| status.is_informational()) =>
                        {
                            // This is the ordering boundary: the codec has room and
                            // these HEADERS are the next frame to be buffered. Later
                            // SETTINGS ACKs cannot overtake them. Do not snapshot
                            // streams merely because they have been queued.
                            let snapshot = if counts.peer().is_server() {
                                webtransport.peer_acknowledged()
                            } else {
                                webtransport.peer_webtransport_settings()
                            };
                            if !counts.peer().is_server() && snapshot.is_none() {
                                // The peer withdrew support before CONNECT was
                                // emitted. Retire locally; RST_STREAM on this still
                                // idle wire stream would itself be a protocol error.
                                self.clear_queue(buffer, &mut stream);
                            ...
                            stream.webtransport_outbound_snapshot = Some(snapshot);
                            stream.notify_send();
