    ...
    P: Peer,
{
        ...
        T: AsyncWrite + Unpin,
    {
        loop {
            ...
            let status = {
                let mut me = self.inner.lock().unwrap();
                let status = me.buffer_pending(&self.send_buffer, dst, webtransport)?;
    ...
    pub fn send_request(
        ...
        stream.webtransport_outbound = is_webtransport;
...
impl<B> DynStreams<'_, B> {
    ...
    pub fn recv_headers(
        &mut self,
        frame: frame::Headers,
        settings: crate::webtransport::ReceivedSettingsSnapshot,
    ) -> Result<(), Error> {
        let mut me = self.inner.lock().unwrap();

        me.recv_headers(self.peer, self.send_buffer, frame, settings)
    }
    ...
    fn recv_headers<B>(
        ...
        self.counts.transition(stream, |counts, stream| {
            ...
            let res = if stream.state.is_recv_headers() {
                match actions.recv.recv_headers(frame, stream, counts, settings) {
                    Ok(()) => Ok(()),
...
impl<B> StreamRef<B> {
    ...
    pub fn take_request(&self) -> Request<()> {
        ...
        stream.webtransport_outbound = request
            .extensions()
    ...
    pub fn poll_webtransport_peer_settings(
        &mut self,
        cx: &Context<'_>,
    ) -> Poll<Result<Option<crate::webtransport::SettingsSnapshot>, crate::Error>> {
        let mut me = self.opaque.inner.lock().unwrap();
        let mut stream = me.store.resolve(self.opaque.key);
        if let Some(reason) = stream.state.ensure_reason(proto::PollReset::Streaming)? {
            return Poll::Ready(Err(reason.into()));
        }
        if let Some(snapshot) = stream.webtransport_outbound_snapshot {
...
impl OpaqueStreamRef {
    pub fn webtransport_outbound_snapshot(
        &self,
    ) -> Option<Option<crate::webtransport::SettingsSnapshot>> {
        let mut me = self.inner.lock().ok()?;
        Some(me.store.resolve(self.key).webtransport_outbound_snapshot?)
    }
