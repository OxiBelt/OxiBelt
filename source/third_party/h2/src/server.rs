...
where
  ...
  B: Buf,
{
  /// Returns acknowledged WebTransport SETTINGS for this connection.
  pub fn webtransport_settings(&self) -> crate::webtransport::SettingsHandle {
  ...
  fn handshake2(io: T, builder: Builder) -> Handshake<T, B> {
    ...
    let state = Handshaking::Flushing(Flush::new(codec).instrument(tracing::trace_span!("flush")));
  ...
  /// Sends a WebTransport SETTINGS update.
  ///
  /// The update becomes effective only after the peer acknowledges it.
  pub fn set_webtransport_settings(
    &mut self,
    settings: crate::webtransport::Settings,
  ) -> Result<(), crate::Error> {
    self.connection.set_webtransport_settings(settings)?;
    Ok(())
  }
...
impl Builder {
  ...
  /// Configures the WebTransport HTTP/2 SETTINGS sent during the handshake.
  pub fn webtransport_settings(&mut self, settings: crate::webtransport::Settings) -> &mut Self {
    self.settings.set_webtransport_settings(settings);
    self
  }
  ...
  B: Buf,
{
  ...
  fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
    ...
    while rem > 0 {
      ...
      ready!(Pin::new(self.inner_mut()).poll_read(cx, &mut buf)).map_err(crate::Error::from_io)?;
    ...
    loop {
      match &mut self.state {
        ...
        Handshaking::ReadingPreface(read) => {
          ...
          let webtransport = crate::webtransport::SettingsHandle::new();
          let connection = proto::Connection::new(
            ...
            Config {
              ...
              local_error_reset_streams_max: self.builder.local_max_error_reset_streams,
  ...
  fn convert_poll_message(
    ...
    if let Some(path) = pseudo.path {
      ...
      parts.path_and_query = Some(
        maybe_path
          .or_else(|why| malformed!("malformed headers: malformed path ({:?}): {}", path, why,))?,
      );
    } else if is_connect && has_protocol {
