...
impl<B: Buf> SendStream<B> {
    ...
    /// Waits for the WebTransport CONNECT request or final response HEADERS
    /// to be buffered in wire order, returning the peer SETTINGS acknowledged
    /// before those HEADERS. Later SETTINGS updates do not change this receipt.
    ///
    /// Call only for a WebTransport stream after sending its initial headers.
    /// A reset or connection failure returns an error.
    pub fn poll_webtransport_peer_settings(
        &mut self,
        cx: &mut Context<'_>,
    ) -> Poll<Result<Option<crate::webtransport::SettingsSnapshot>, crate::Error>> {
        self.inner.poll_webtransport_peer_settings(cx)
    }
