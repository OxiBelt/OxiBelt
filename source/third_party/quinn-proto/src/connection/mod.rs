...
pub use streams::{
    Chunks, ClosedStream, FinishError, ReadError, ReadableError, RecvStream, ResetAtError, ResetInfo, SendStream,
...
impl Connection {
    ...
    /// Whether the peer advertised draft-09 RESET_STREAM_AT support.
    pub fn peer_supports_reset_stream_at(&self) -> bool {
        self.peer_params.reset_stream_at
    }
    ...
    fn on_packet_acked(&mut self, now: Instant, info: SentPacket) {
        ...
        if let Some(retransmits) = info.retransmits.get() {
            ...
            for frame in retransmits.reset_stream_at.iter() {
                self.streams.reset_at_acked(*frame);
            }
        }
