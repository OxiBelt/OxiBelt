...
impl SendBuffer {
    ...
    /// Stop sending data beyond `reliable_size`, retaining the prefix for retransmission.
    pub(super) fn reset_at(&mut self, reliable_size: u64) {
        self.retransmits.remove(reliable_size..self.offset);
        if self.unsent < reliable_size {
            self.retransmits.insert(self.unsent..reliable_size);
        }
        self.unsent = self.offset;
    }

    pub(super) fn prefix_acked(&self, reliable_size: u64) -> bool {
        self.offset - self.unacked_len as u64 >= reliable_size
    }
...
mod tests {
    ...
    #[test]
    fn partial_reset_only_retransmits_the_reliable_prefix() {
        let mut buffer = SendBuffer::new();
        buffer.write(Bytes::from_static(b"headerbody"));
        assert_eq!(buffer.poll_transmit(32).0, 0..10);
        buffer.retransmit(0..10);
        buffer.reset_at(6);
        assert_eq!(buffer.poll_transmit(32).0, 0..6);
        assert!(!buffer.has_unsent_data());
        buffer.ack(0..6);
        assert!(buffer.prefix_acked(6));
    }
