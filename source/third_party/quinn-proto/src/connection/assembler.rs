...
pub(super) struct Assembler {
    ...
    /// Last reliable-size boundary at which buffered data was physically trimmed.
    /// Reads use the current RESET_STREAM_AT size even between trims.
    physical_discard_limit: Option<u64>,
}
...
impl Assembler {
    ...
    pub(super) fn read(&mut self, max_length: usize, ordered: bool) -> Option<Chunk> {
        self.read_bounded(max_length, ordered, None)
    }

    pub(super) fn read_bounded(&mut self, max_length: usize, ordered: bool, limit: Option<u64>) -> Option<Chunk> {
        loop {
            let mut chunk = self.data.peek_mut()?;

            if limit.is_some_and(|limit| chunk.offset >= limit) {
            ...
            let max_length = limit.map_or(max_length, |limit| max_length.min((limit - chunk.offset) as usize));
    ...
    /// Whether the application has received every byte below `limit`.
    pub(super) fn delivered_prefix(&self, limit: u64) -> bool {
        if limit == 0 {
            return true;
        }
        match &self.state {
            State::Ordered => self.bytes_read >= limit,
            State::Unordered { recvd } => {
                recvd.iter().next().is_some_and(|range| range.start == 0 && range.end >= limit)
                    && self.data.peek().is_none_or(|chunk| chunk.offset >= limit)
            }
        }
    }

    /// Release buffered data beyond a partial reset's reliable size.
    pub(super) fn discard_after(&mut self, limit: u64) {
        // RESET_STREAM_AT can be repeated with a one-byte-smaller Reliable Size.
        // Rebuilding the heap and copying the retained boundary on every frame
        // would be quadratic. Keep at most one extra factor of two of buffered
        // prefix span and physically trim only when that span halves.
        if self.physical_discard_limit.is_some_and(|old| limit > old / 2) {
            return;
        }
        self.physical_discard_limit = Some(limit);
        let mut retained = BinaryHeap::new();
        self.buffered = 0;
        self.allocated = 0;
        for mut chunk in mem::take(&mut self.data).into_vec() {
            if chunk.offset >= limit {
                continue;
            }
            let keep = (limit - chunk.offset).min(chunk.bytes.len() as u64) as usize;
            if keep < chunk.bytes.len() {
                chunk = Buffer::new_defragmented(chunk.offset, Bytes::copy_from_slice(&chunk.bytes[..keep]));
            }
            self.buffered += chunk.bytes.len();
            self.allocated += chunk.allocation_size;
            retained.push(chunk);
    ...
    pub(super) fn clear(&mut self) {
        ...
        self.buffered = 0;
        self.allocated = 0;
        self.physical_discard_limit = None;
    }
}
...
mod test {
    ...
    #[test]
    fn repeated_descending_reliable_sizes_amortize_boundary_copy() {
        const SIZE: usize = 256 * 1024;
        let mut assembler = Assembler::new();
        assembler.insert(0, Bytes::from(vec![7; SIZE]), SIZE).unwrap();
        assembler.discard_after(SIZE as u64);
        let original = assembler.data.peek().unwrap().bytes.as_ptr();

        // Each one-byte decrease is a valid RESET_STREAM_AT update. None of
        // these updates should copy the same large retained prefix again.
        for limit in (SIZE - 1024..SIZE).rev() {
            assembler.discard_after(limit as u64);
            assert_eq!(assembler.data.peek().unwrap().bytes.as_ptr(), original);
            assert_eq!(assembler.buffered, SIZE);
            assert_eq!(assembler.allocated, SIZE);
        }
        assembler.discard_after((SIZE / 2) as u64);
        assert_eq!(assembler.buffered, SIZE / 2);
        assert_eq!(assembler.allocated, SIZE / 2);
        let half = assembler.data.peek().unwrap().bytes.as_ptr();
        assert_ne!(half, original);
        for limit in (SIZE / 2 - 1024..SIZE / 2).rev() {
            assembler.discard_after(limit as u64);
            assert_eq!(assembler.data.peek().unwrap().bytes.as_ptr(), half);
        }
        assembler.discard_after(0);
        assert!(assembler.data.is_empty());
        assert_eq!(assembler.buffered, 0);
        assert_eq!(assembler.allocated, 0);
    }

    #[test]
    fn unordered_read_obeys_latest_logical_reliable_size() {
        let mut assembler = Assembler::new();
        assembler.insert(0, Bytes::from(vec![7; 1024]), 1024).unwrap();
        assembler.ensure_ordering(false).unwrap();
        assembler.discard_after(1024);
        assembler.discard_after(1023); // logical shrink without a physical trim
        assert_eq!(assembler.buffered, 1024);
        let chunk = assembler.read_bounded(1024, false, Some(1023)).unwrap();
        assert_eq!(chunk.bytes.len(), 1023);
        assert!(assembler.delivered_prefix(1023));
        assert!(assembler.read_bounded(1024, false, Some(1023)).is_none());
        assert_eq!(assembler.buffered, 0);
    }
