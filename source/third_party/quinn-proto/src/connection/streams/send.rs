...
impl Send {
    ...
    pub(super) fn reset(&mut self) {
        use SendState::*;
        if let DataSent { .. } | Ready | ResetAt { .. } = self.state {
    ...
    pub(super) fn reset_at(&mut self, error_code: VarInt, reliable_size: u64) {
        self.pending.reset_at(reliable_size);
        self.state = SendState::ResetAt {
            error_code,
            reliable_size,
            reset_acked: false,
        };
    ...
    pub(super) fn ack(&mut self, frame: frame::StreamMeta) -> bool {
        ...
        match self.state {
            ...
            SendState::ResetAt {
                ...
                reset_acked,
                ..
            } => reset_acked && self.pending.prefix_acked(reliable_size),
            _ => false,
        }
    }
...
pub(super) enum SendState {
    ...
    /// Sent RESET
    ResetSent,
    /// RESET_STREAM_AT has been sent; prefix and reset are both awaited.
    ResetAt {
        error_code: VarInt,
        reliable_size: u64,
        reset_acked: bool,
