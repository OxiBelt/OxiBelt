...
/// Metadata sent with a peer stream reset.
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub struct ResetInfo {
    /// Application protocol error code.
    pub error_code: VarInt,
    /// Final size, used for stream and connection flow control accounting.
    pub final_size: u64,
    /// Prefix guaranteed to be delivered by RESET_STREAM_AT, if used.
...
/// Failure to request a partial-delivery stream reset.
#[derive(Debug, Error, Clone, Copy, Eq, PartialEq)]
pub enum ResetAtError {
    /// The peer has not advertised support for partial-delivery resets.
    #[error("peer did not negotiate RESET_STREAM_AT")]
    Unsupported,
    /// The requested prefix is invalid or would increase a previous guarantee.
    #[error("reliable size exceeds bytes written or a prior reliable size")]
    InvalidReliableSize,
...
impl RecvStream<'_> {
    ...
    pub fn received_reset(&mut self) -> Result<Option<VarInt>, ClosedStream> {
        self.received_reset_info().map(|info| info.map(|info| info.error_code))
    }

    /// Inspect reset metadata without consuming the receive stream.
    pub fn reset_info(&self) -> Option<ResetInfo> {
        self.state.recv.get(&self.id).and_then(|stream| stream.as_ref())
            .and_then(StreamRecv::as_open_recv).and_then(Recv::reset_info)
    }

    /// Awaitable reset metadata for peers that reset the stream.
    pub fn received_reset_info(&mut self) -> Result<Option<ResetInfo>, ClosedStream> {
        ...
        let Some(info) = s.reset_info() else {
            return Ok(None);
        };

        // A partial reset leaves a prefix readable. Report the code without discarding it.
        if s.reliable_size().is_some() {
            return Ok(Some(info));
        }
...
impl<'a> SendStream<'a> {
    ...
    pub fn reset(&mut self, error_code: VarInt) -> Result<(), ClosedStream> {
        ...
        if let SendState::ResetAt { error_code: original, .. } = stream.state {
            if error_code != original {
                return Err(ClosedStream { _private: () });
            }
        }
        ...
        stream.reset();
        self.pending.reset_stream_at.retain(|frame| frame.id != self.id);
    ...
    /// Reset a stream while guaranteeing delivery of its first `reliable_size` bytes.
    ///
    /// The peer must advertise draft-ietf-quic-reliable-stream-reset-09 support.
    pub fn reset_at(&mut self, error_code: VarInt, reliable_size: u64) -> Result<(), ResetAtError> {
        if !self.state.peer_reset_stream_at {
            return Err(ResetAtError::Unsupported);
        }
        let max_send_data = self.state.max_send_data(self.id);
        let stream = self.state.send.get_mut(&self.id)
            .map(get_or_insert_send(max_send_data))
            .ok_or(ResetAtError::ClosedStream)?;
        if reliable_size > stream.offset() || VarInt::try_from(reliable_size).is_err() {
            return Err(ResetAtError::InvalidReliableSize);
        }
        match stream.state {
            SendState::Ready | SendState::DataSent { .. } => {}
            SendState::ResetAt { error_code: original, reliable_size: old, .. }
                if original == error_code && reliable_size <= old => {}
            SendState::ResetAt { .. } => return Err(ResetAtError::InvalidReliableSize),
            SendState::ResetSent => return Err(ResetAtError::ClosedStream),
        }
        let was_pending = stream.is_pending();
        stream.reset_at(error_code, reliable_size);
        if !was_pending && stream.is_pending() {
            self.state.pending.push_pending(self.id, stream.priority);
        }
        self.pending.reset_stream_at.push(frame::ResetStreamAt {
            id: self.id,
            error_code,
            final_offset: VarInt::try_from(stream.offset()).expect("impossibly large offset"),
            reliable_offset: VarInt::try_from(reliable_size).expect("checked above"),
        });
        Ok(())
    }
