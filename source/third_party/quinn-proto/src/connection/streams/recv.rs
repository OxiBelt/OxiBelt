...
use super::{ClosedStream, ResetInfo, Retransmits, ShouldTransmit, StreamId, StreamsState};
...
impl Recv {
    ...
    pub(super) fn ingest(
        ...
        if !self.stopped {
            let fin = frame.fin;
            let mut frame = frame;
            if let RecvState::ResetAt { reliable_size, .. } = self.state {
                frame.data.truncate(reliable_size.saturating_sub(frame.offset).min(frame.data.len() as u64) as usize);
            }
            self.assembler
                .insert(frame.offset, frame.data, payload_len)
                .map_err(|_| TransportError::INTERNAL_ERROR("too many gaps in stream buffer"))?;
            return Ok((new_bytes, fin && self.stopped));
        }
    ...
    pub(super) fn stop(&mut self) -> Result<(u64, ShouldTransmit), ClosedStream> {
        ...
        let credit_end = self.reliable_size().unwrap_or(self.end);
        let read_credits = credit_end.saturating_sub(self.assembler.bytes_read());
    ...
    pub(super) fn is_receiving(&self) -> bool {
        matches!(self.state, RecvState::Recv { .. } | RecvState::ResetAt { .. })
    }
    ...
    fn final_offset(&self) -> Option<u64> {
        match self.state {
            ...
            RecvState::ResetAt { size, .. } => Some(size),
        }
    }
    ...
    pub(super) fn reset(
        ...
        if let RecvState::ResetAt { error_code: original, .. } | RecvState::ResetRecvd { error_code: original, .. } = self.state {
            if original != error_code {
                return Err(TransportError::STREAM_STATE_ERROR("inconsistent reset error code"));
            }
        }
    ...
    /// Process a partial-delivery reset. Returns false for redundant frames.
    pub(super) fn reset_at(
        &mut self,
        error_code: VarInt,
        final_offset: VarInt,
        reliable_offset: VarInt,
        received: u64,
        max_data: u64,
    ) -> Result<bool, TransportError> {
        let reliable_size = reliable_offset.into_inner();
        let size = final_offset.into_inner();
        if reliable_size > size {
            return Err(TransportError::FRAME_ENCODING_ERROR("reliable size exceeds final size"));
        }
        if let Some(old_size) = self.final_offset() {
            if old_size != size {
                return Err(TransportError::FINAL_SIZE_ERROR("inconsistent value"));
            }
        } else if self.end > size {
            return Err(TransportError::FINAL_SIZE_ERROR("lower than high water mark"));
        }
        self.credit_consumed_by(size, received, max_data)?;
        match self.state {
            RecvState::ResetRecvd { error_code: old, .. } => {
                if old != error_code {
                    return Err(TransportError::STREAM_STATE_ERROR("inconsistent reset error code"));
                }
                return Ok(false);
            }
            RecvState::ResetAt { error_code: old, reliable_size: old_reliable, .. } => {
                if old != error_code {
                    return Err(TransportError::STREAM_STATE_ERROR("inconsistent reset error code"));
                }
                if reliable_size >= old_reliable {
                    return Ok(false);
                }
            }
            RecvState::Recv { .. } => {}
        }
        self.state = RecvState::ResetAt { size, reliable_size, error_code };
        self.assembler.discard_after(reliable_size);
        // Charge the whole final size once, before more prefix data arrives.
        self.end = size;
        Ok(true)
    }

    pub(super) fn reset_info(&self) -> Option<ResetInfo> {
        match self.state {
            RecvState::ResetRecvd { size, error_code } => Some(ResetInfo {
                error_code, final_size: size, reliable_size: None,
            }),
            RecvState::ResetAt { size, reliable_size, error_code } => Some(ResetInfo {
                error_code, final_size: size, reliable_size: Some(reliable_size),
            }),
            _ => None,
        }
    }

    pub(super) fn reliable_size(&self) -> Option<u64> {
        match self.state {
            RecvState::ResetAt { reliable_size, .. } => Some(reliable_size),
            _ => None,
        }
    }
...
impl<'a> Chunks<'a> {
    ...
    pub fn next(&mut self, max_length: usize) -> Result<Option<Chunk>, ReadError> {
        ...
        let limit = match rs.state {
            RecvState::ResetAt { reliable_size, error_code, .. }
                if rs.assembler.delivered_prefix(reliable_size) => {
                    let state = mem::replace(&mut self.state, ChunksState::Reset(error_code));
                    let ChunksState::Readable(recv) = state else { unreachable!() };
                    self.streams.stream_recv_freed(self.id, StreamRecv::Open(recv));
                    return Err(ReadError::Reset(error_code));
                }
            RecvState::ResetAt { reliable_size, .. } => Some(reliable_size),
            _ => None,
        };
        if let Some(chunk) = rs.assembler.read_bounded(max_length, self.ordered, limit) {
...
enum RecvState {
    ...
    ResetAt { size: u64, reliable_size: u64, error_code: VarInt },
}
...
mod tests {
    ...
    #[test]
    fn descending_partial_resets_keep_latest_logical_size_and_flow_credit() {
        const SIZE: u32 = 256 * 1024;
        let mut recv = Recv::new(SIZE as u64);
        let (charged, closed) = recv
            .ingest(
                frame::Stream {
                    id: StreamId::new(Side::Client, Dir::Uni, 0),
                    offset: 0,
                    fin: false,
                    data: Bytes::from(vec![7; SIZE as usize]),
                },
                SIZE as usize,
                0,
                SIZE as u64,
            ...
            .unwrap();
        assert_eq!(charged, SIZE as u64);
        assert!(!closed);

        let code = VarInt::from_u32(95);
        let final_size = VarInt::from_u32(SIZE);
        for reliable in (SIZE - 1024..=SIZE).rev() {
            assert!(recv
                .reset_at(code, final_size, VarInt::from_u32(reliable), SIZE as u64, SIZE as u64)
                .unwrap());
            assert_eq!(recv.reset_info().unwrap().reliable_size, Some(reliable as u64));
            assert_eq!(recv.end, SIZE as u64);
        }
        let (credit, _) = recv.stop().unwrap();
        assert_eq!(credit, (SIZE - 1024) as u64);
        assert!(recv.assembler.read_bounded(SIZE as usize, true, Some(SIZE as u64)).is_none());
    }
