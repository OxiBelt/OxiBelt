...
impl StreamsState {
    ...
    pub fn received_reset(
        ...
        let prior_reliable = rs.reliable_size();
        // State transition
        if !rs.reset(
            error_code,
            final_offset,
        ...
        Ok(if credited != final_offset.into_inner() {
            ...
            self.add_read_credits(prior_reliable.unwrap_or(u64::from(final_offset)).saturating_sub(credited))
        } else {
            ShouldTransmit(false)
        })
    }

    /// Process draft-09 RESET_STREAM_AT and keep the reliable prefix readable.
    pub(crate) fn received_reset_at(
        &mut self,
        frame: frame::ResetStreamAt,
    ) -> Result<ShouldTransmit, TransportError> {
        self.validate_receive_id(frame.id)?;
        let rs = match self.recv.get_mut(&frame.id)
            .map(get_or_insert_recv(self.stream_receive_window)) {
            Some(stream) => stream,
            None => return Ok(ShouldTransmit(false)),
        };
        let old_end = rs.end;
        let old_reliable = rs.reliable_size();
        let bytes_read = rs.assembler.bytes_read();
        if !rs.reset_at(frame.error_code, frame.final_offset, frame.reliable_offset,
                        self.data_recvd, self.local_max_data)? {
            return Ok(ShouldTransmit(false));
        }
        let stopped = rs.stopped;
        let new_reliable = frame.reliable_offset.into_inner();
        let credit = if stopped {
            frame.final_offset.into_inner().saturating_sub(old_end)
        } else if let Some(old_reliable) = old_reliable {
            old_reliable.max(bytes_read).saturating_sub(new_reliable.max(bytes_read))
        } else {
            frame.final_offset.into_inner().saturating_sub(new_reliable.max(bytes_read))
        };
        self.data_recvd += frame.final_offset.into_inner().saturating_sub(old_end);
        if stopped {
            let rs = self.recv.remove(&frame.id).flatten().unwrap();
            self.stream_recv_freed(frame.id, rs);
        }
        self.on_stream_frame(!stopped, frame.id);
    ...
    pub(crate) fn reset_at_acked(&mut self, frame: frame::ResetStreamAt) {
        let hash_map::Entry::Occupied(mut entry) = self.send.entry(frame.id) else {
            return;
        };
        let Some(stream) = entry.get_mut().as_mut() else {
            return;
        };
        if let SendState::ResetAt { reliable_size, ref mut reset_acked, .. } = stream.state {
            if reliable_size == frame.reliable_offset.into_inner() {
                *reset_acked = true;
                if stream.pending.prefix_acked(reliable_size) {
                    self.buffered_data -= stream.pending.buffered();
                    entry.remove_entry();
                    self.stream_freed(frame.id, StreamHalf::Send);
                    self.events.push_back(StreamEvent::Finished { id: frame.id });
                }
            }
        }
    }
    ...
    pub(in crate::connection) fn write_control_frames(
        ...
        while buf.len() + frame::ResetStreamAt::SIZE_BOUND < max_size {
            let Some(frame) = pending.reset_stream_at.pop() else { break; };
            let Some(stream) = self.send.get(&frame.id).and_then(|s| s.as_ref()) else {
                continue;
            };
            if !matches!(stream.state, SendState::ResetAt { reliable_size, .. } if reliable_size == frame.reliable_offset.into_inner()) {
                continue;
            }
            retransmits.get_or_create().reset_stream_at.push(frame);
            frame.encode(buf);
            stats.reset_stream += 1;
        }
    ...
    pub(crate) fn write_stream_frames(
        ...
        while buf.len() + frame::Stream::SIZE_BOUND < max_buf_size {
            ...
            // A partial reset can abandon all queued data (reliable size zero), leaving a
            // stale entry in the send queue.
    ...
    pub(crate) fn retransmit(&mut self, frame: frame::StreamMeta) {
        ...
        if let SendState::ResetAt { reliable_size, .. } = stream.state {
            offsets.end = offsets.end.min(reliable_size);
        ...
        stream.fin_pending |= frame.fin && !matches!(stream.state, SendState::ResetAt { .. });
    ...
    pub(crate) fn retransmit_all_for_0rtt(&mut self) {
        for dir in Dir::iter() {
            for index in 0..self.next[dir as usize] {
                ...
                // A Retry can rewind data already sent in 0-RTT. Preserve the partial reset's
                // abandoned suffix; only the reliable prefix is eligible for retransmission.
                if let SendState::ResetAt { reliable_size, .. } = stream.state {
                    stream.pending.reset_at(reliable_size);
                }
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn reset_at_before_data_waits_for_prefix_and_checks_sizes() {
        ...
        let frame = frame::ResetStreamAt {
            ...
            reliable_offset: VarInt(6),
        };
        let _ = client.received_reset_at(frame).unwrap();
        let mut pending = Retransmits::default();
        let mut recv = RecvStream { id, state: &mut client, pending: &mut pending };
        assert_eq!(recv.received_reset(), Ok(Some(VarInt(42))));
        assert_eq!(recv.reset_info(), Some(crate::ResetInfo {
            error_code: VarInt(42), final_size: 10, reliable_size: Some(6),
        }));
        let mut chunks = recv.read(true).unwrap();
        assert_eq!(chunks.next(16), Err(ReadError::Blocked));
        let _ = chunks.finalize();

        // The later prefix segment arrives before the first one.
        let _ = client.received(frame::Stream {
            id, offset: 3, fin: false, data: Bytes::from_static(b"der")
        }, 3).unwrap();
        let mut recv = RecvStream { id, state: &mut client, pending: &mut pending };
        let mut chunks = recv.read(true).unwrap();
        assert_eq!(chunks.next(16), Err(ReadError::Blocked));
        let _ = chunks.finalize();
        let _ = client.received(frame::Stream {
            id, offset: 0, fin: false, data: Bytes::from_static(b"hea")
        }, 3).unwrap();
        let mut recv = RecvStream { id, state: &mut client, pending: &mut pending };
        let mut chunks = recv.read(true).unwrap();
        assert_eq!(&chunks.next(3).unwrap().unwrap().bytes[..], b"hea");
        let _ = chunks.finalize();
        assert_eq!(client.recv.get(&id).unwrap().as_ref().unwrap().as_open_recv().unwrap().reset_info(),
            Some(crate::ResetInfo { error_code: VarInt(42), final_size: 10, reliable_size: Some(6) }));
        let mut recv = RecvStream { id, state: &mut client, pending: &mut pending };
        let mut chunks = recv.read(true).unwrap();
        assert_eq!(&chunks.next(16).unwrap().unwrap().bytes[..], b"der");
        assert_eq!(chunks.next(16), Err(ReadError::Reset(VarInt(42))));
        let _ = chunks.finalize();

        let bad = frame::ResetStreamAt { reliable_offset: VarInt(11), ..frame };
        let mut other = make(Side::Client);
        assert_eq!(other.received_reset_at(bad).unwrap_err().code, TransportError::FRAME_ENCODING_ERROR("").code);
    }

    #[test]
    fn retry_rewinds_only_the_reliable_reset_prefix() {
        let mut client = make(Side::Client);
        ...
        let id = Streams { state: &mut client, conn_state: &state }.open(Dir::Uni).unwrap();
        let mut send = SendStream { id, state: &mut client, pending: &mut pending, conn_state: &state };
        send.write(b"headerbody").unwrap();
        assert_eq!(client.send.get_mut(&id).unwrap().as_mut().unwrap().pending.poll_transmit(32).0, 0..10);
        SendStream { id, state: &mut client, pending: &mut pending, conn_state: &state }
            .reset_at(42u32.into(), 6).unwrap();

        client.retransmit_all_for_0rtt();
        let buffer = &mut client.send.get_mut(&id).unwrap().as_mut().unwrap().pending;
        assert_eq!(buffer.poll_transmit(32).0, 0..6);
        assert!(!buffer.has_unsent_data());
    }
    use crate::{
        ReadError, ReadableError, RecvStream, SendStream, TransportErrorCode, WriteError,
