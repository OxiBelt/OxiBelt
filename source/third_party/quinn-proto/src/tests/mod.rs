...
#[test]
fn reset_stream_at_delivers_prefix_then_error() {
    let mut pair = Pair::default();
    let (client_ch, server_ch) = pair.connect();
    assert!(pair.client_conn_mut(client_ch).peer_supports_reset_stream_at());
    assert!(pair.server_conn_mut(server_ch).peer_supports_reset_stream_at());

    let id = pair.client_streams(client_ch).open(Dir::Uni).unwrap();
    let mut send = pair.client_send(client_ch, id);
    send.write(b"headerbody").unwrap();
    assert_eq!(send.reset_at(VarInt(42), 6), Ok(()));
    pair.drive();

    assert_matches!(pair.server_conn_mut(server_ch).poll(),
        Some(Event::Stream(StreamEvent::Opened { dir: Dir::Uni })));
    assert_eq!(pair.server_streams(server_ch).accept(Dir::Uni), Some(id));
    let mut recv = pair.server_recv(server_ch, id);
    assert_eq!(recv.received_reset(), Ok(Some(VarInt(42))));
    let mut chunks = recv.read(true).unwrap();
    assert_matches!(chunks.next(usize::MAX), Ok(Some(chunk)) if &chunk.bytes[..] == b"header");
    assert_eq!(chunks.next(usize::MAX), Err(ReadError::Reset(VarInt(42))));
    let _ = chunks.finalize();
    assert_eq!(pair.client_streams(client_ch).send_streams(), 0);
}
