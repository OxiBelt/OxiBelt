...
macro_rules! make_struct {
    {$($(#[$doc:meta])* $name:ident ($id:ident) = $default:expr,)*} => {
        ...
        pub struct TransportParameters {
            ...
            /// Support for draft-ietf-quic-reliable-stream-reset-09 RESET_STREAM_AT.
...
mod test {
    ...
    #[test]
    fn reset_stream_at_is_empty_and_persisted_for_zero_rtt() {
        let mut encoded = Vec::new();
        let mut params = TransportParameters::default();
        params.reset_stream_at = true;
        params.write(&mut encoded);
        assert_eq!(TransportParameters::read(Side::Client, &mut encoded.as_slice()).unwrap().reset_stream_at, true);
        assert!(TransportParameters::default().validate_resumption_from(&params).is_err());

        let nonempty = [0x1d, 0x01, 0x00];
        assert_eq!(TransportParameters::read(Side::Client, &mut nonempty.as_slice()), Err(Error::Malformed));
        let duplicate = [0x1d, 0x00, 0x1d, 0x00];
        assert_eq!(TransportParameters::read(Side::Client, &mut duplicate.as_slice()), Err(Error::Malformed));
    }
