...
impl Iter {
    ...
    fn try_next(&mut self) -> Result<Frame, IterErr> {
        ...
        Ok(match ty {
            ...
            FrameType::RESET_STREAM_AT => Frame::ResetStreamAt(ResetStreamAt {
                id: self.bytes.get()?,
                error_code: self.bytes.get()?,
                final_offset: self.bytes.get()?,
                reliable_offset: self.bytes.get()?,
            }),
...
/// draft-ietf-quic-reliable-stream-reset-09 partial-delivery reset.
#[derive(Debug, Copy, Clone)]
pub(crate) struct ResetStreamAt {
    pub(crate) id: StreamId,
    pub(crate) error_code: VarInt,
    pub(crate) final_offset: VarInt,
    pub(crate) reliable_offset: VarInt,
}
...
impl ResetStreamAt {
    pub(crate) fn encode<W: BufMut>(&self, out: &mut W) {
        ...
        out.write(self.reliable_offset);
    }
}
...
mod test {
    ...
    #[test]
    fn reset_stream_at_coding() {
        let frame = ResetStreamAt {
            id: StreamId::new(crate::Side::Client, Dir::Uni, 3),
        ...
        assert_eq!(buf[0], FrameType::RESET_STREAM_AT.0 as u8);
        let decoded = frames(buf);
        assert_matches!(&decoded[0], Frame::ResetStreamAt(got)
            if got.id == frame.id && got.error_code == frame.error_code
                && got.final_offset == frame.final_offset && got.reliable_offset == frame.reliable_offset);
    }
