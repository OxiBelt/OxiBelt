  ...
  T: Http1Transaction,
{
  ...
  pub(crate) fn poll_read_body(
    ...
    let (reading, ret) = match &mut self.state.reading {
      ...
      Reading::Continue(decoder) => {
        ...
        #[cfg(feature = "server")]
        let continue_sent = self.state.continue_sent;
        #[cfg(not(feature = "server"))]
        let continue_sent = false;
  ...
  fn maybe_notify(&mut self, cx: &mut Context<'_>) {
    ...
    match self.state.reading {
      Reading::Continue(..) | Reading::Body(..) | Reading::KeepAlive | Reading::Closed => return,
      Reading::Init => (),
    }
  ...
  pub(crate) fn continue_sent(&self) -> bool {
    #[cfg(feature = "server")]
    {
  ...
  pub(crate) fn mark_continue_sent(&mut self) {
    #[cfg(feature = "server")]
    {
      self.state.continue_sent = true;
    }
  ...
  /// Encode an informational server response without completing the response
  /// half of the current exchange.
  ///
  /// The ordinary `write_head` path advances `Writing::Init` to
  /// `Writing::KeepAlive` for a zero-length response and caches the drained
  /// header map for the next message. Neither transition is valid for a 1xx
  /// head: the service must remain able to send more informational responses
  /// and its final response while the request body is still arriving.
  pub(crate) fn write_informational_head(&mut self, mut head: MessageHead<T::Outgoing>) {
    debug_assert!(self.can_write_head());

    // Always serialize with the downstream HTTP/1 version. A relayed response
    // can retain its upstream HTTP/2 or HTTP/3 version, which is not a valid
    // HTTP/1 status-line version and must not reach the role encoder. Do not
    // call `enforce_version`: final-response keep-alive mutations such as
    // `Connection: close` are forbidden on informational responses.
    head.version = if self.state.version == Version::HTTP_10 {
      Version::HTTP_10
    } else {
      Version::HTTP_11
    };
    ...
    match super::role::encode_headers::<T>(
      ...
      Ok(encoder) => {
        debug_assert!(encoder.is_eof());
        debug_assert!(!encoder.is_last());
  ...
  fn encode_head(
    ...
    match super::role::encode_headers::<T>(
      ...
      Ok(encoder) => {
        ...
        #[cfg(feature = "client")]
        {
          self.state.on_informational = head.extensions.remove::<crate::ext::OnInformational>();
        }
  ...
  pub(crate) fn write_trailers(&mut self, trailers: HeaderMap) {
    ...
    match &mut self.state.writing {
      Writing::Body(encoder) => {
        if let Some(enc_buf) = encoder.encode_trailers(trailers, self.state.title_case_headers) {
...
struct State {
  ...
  /// A `100 Continue` has already been written for the current request.
  /// This covers both Hyper's automatic `Expect: 100-continue` response and
  /// a service-provided informational response.
  #[cfg(feature = "server")]
  continue_sent: bool,
