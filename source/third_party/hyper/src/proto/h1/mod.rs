...
pub(crate) trait Http1Transaction {
    ...
    /// Whether an outgoing head is the HTTP/1.1 `100 Continue` response.
    ///
    /// The dispatcher is generic over client and server transactions, so this
    /// role-specific predicate keeps the server-only status comparison out of
    /// the generic write loop.
