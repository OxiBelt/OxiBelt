...
pub(crate) trait Dispatch {
  ...
  fn poll_informational(
    self: Pin<&mut Self>,
    cx: &mut Context<'_>,
  ) -> Poll<Option<Result<Self::PollItem, Self::PollError>>>;
  ...
  Bs::Error: Into<Box<dyn StdError + Send + Sync>>,
{
  ...
  fn poll_write(&mut self, cx: &mut Context<'_>) -> Poll<crate::Result<()>> {
    loop {
      ...
      {
        if self.pump_informational(cx)? {
          continue;
        }
        if let Some(msg) = ready!(Pin::new(&mut self.dispatch).poll_msg(cx)) {
          let (head, body) = msg.map_err(crate::Error::new_user_service)?;

          // A service may enqueue an informational response and resolve its
          // final response in the same poll. Drain every already-queued head
          // before the final one so HTTP/1 wire ordering cannot invert them.
          while self.pump_informational(cx)? {}
          ...
          if let Some(item) = item {
            ...
            if frame.is_data() {
              ...
              *clear_body = true;
              self
                .conn
                .write_trailers(frame.into_trailers().unwrap_or_else(|_| unreachable!()));
            } else {
  ...
  /// Write one queued informational response, if available.
  ///
  /// This is intentionally separate from final response polling: informational
  /// heads do not arm deadlines or change body/final response state.
  fn pump_informational(&mut self, cx: &mut Context<'_>) -> crate::Result<bool> {
    let Poll::Ready(Some(head)) = Pin::new(&mut self.dispatch).poll_informational(cx) else {
      return Ok(false);
    };
    let head = head.map_err(crate::Error::new_user_service)?;
    if T::is_continue_response(&head.subject) {
      if self.conn.continue_sent() {
        // A downstream Expect gate or an earlier relayed 100 already emitted
        // the protocol response. Preserve upstream progress without a
        // duplicate wire head.
        return Ok(true);
      }
      self.conn.mark_continue_sent();
    }
    self.conn.write_informational_head(head);
    Ok(true)
  }
  ...
  fn is_done(&self) -> bool {
    ...
    if !T::should_read_first() && read_done {
      ...
      let write_done =
        self.conn.is_write_closed() || (!self.dispatch.should_poll() && self.body_rx.is_none());
        ...
        Bs: Body,
    {
        ...
        fn poll_informational(
            ...
            match this
                .informational
                .as_mut()
                .expect("in-flight request must retain informational receiver")
                .poll_recv(cx)
            {
                Poll::Ready(Some(response)) => {
                    let (parts, ()) = response.into_parts();
                    Poll::Ready(Some(Ok(MessageHead {
                        version: parts.version,
                        subject: parts.status,
                        headers: parts.headers,
                        extensions: parts.extensions,
        ...
        fn recv_msg(&mut self, msg: crate::Result<(Self::RecvItem, IncomingBody)>) -> crate::Result<()> {
            ...
            *req.extensions_mut() = msg.extensions;
            let (informational, receiver) = crate::ext::informational_channel();
            req.extensions_mut().insert(informational);
...
mod tests {
  ...
  async fn read_http1_head(io: &mut tokio::io::DuplexStream) -> String {
    let mut bytes = Vec::new();
    loop {
      let mut byte = [0u8; 1];
      tokio::time::timeout(Duration::from_secs(1), io.read_exact(&mut byte))
        .await
        .expect("HTTP/1 response head timed out")
        .expect("HTTP/1 response head ended early");
      bytes.push(byte[0]);
      if bytes.ends_with(b"\r\n\r\n") {
        return String::from_utf8(bytes).expect("HTTP/1 response head is UTF-8");
      }
    }
  }

  fn informational_response(status: u16) -> crate::Response<()> {
  ...
  #[tokio::test]
  async fn server_writes_same_poll_informational_before_final_response() {
    let (server_io, mut client_io) = tokio::io::duplex(4096);
    let service = service_fn(|request: crate::Request<IncomingBody>| async move {
      request
        .extensions()
        .get::<crate::ext::InformationalSender>()
        .expect("request informational sender")
        .try_send(informational_response(104))
        .expect("queue informational response");
      Ok::<_, Infallible>(crate::Response::new(Empty::<Bytes>::new()))
    });
    let mut builder = crate::server::conn::http1::Builder::new();
    ...
    client_io
      ...
      .await
      .expect("write HTTP/1 request");
    let mut wire = Vec::new();
    tokio::time::timeout(Duration::from_secs(1), client_io.read_to_end(&mut wire))
      .await
      .expect("HTTP/1 responses timed out")
      .expect("read HTTP/1 responses");
    server
      .await
      .expect("HTTP/1 server task")
      .expect("HTTP/1 server connection");

    let wire = String::from_utf8(wire).expect("HTTP/1 responses are UTF-8");
    let interim = wire.find("HTTP/1.1 104 ").expect("104 response head");
    let final_response = wire.find("HTTP/1.1 200 OK").expect("final response head");
    assert!(interim < final_response, "wire response order: {wire:?}");
  }

  #[tokio::test]
  async fn server_coerces_relayed_http3_informational_to_http1() {
    let (server_io, mut client_io) = tokio::io::duplex(4096);
    let service = service_fn(|request: crate::Request<IncomingBody>| async move {
      let mut informational = informational_response(104);
      *informational.version_mut() = crate::Version::HTTP_3;
      request
        .extensions()
        .get::<crate::ext::InformationalSender>()
        .expect("request informational sender")
        .try_send(informational)
        .expect("queue HTTP/3 informational response");
      Ok::<_, Infallible>(crate::Response::new(Empty::<Bytes>::new()))
    });
    let mut builder = crate::server::conn::http1::Builder::new();
    builder.auto_date_header(false);
  ...
  #[tokio::test]
  async fn server_flushes_informational_while_upload_is_still_pending() {
    let (server_io, mut client_io) = tokio::io::duplex(4096);
    let service = service_fn(|mut request: crate::Request<IncomingBody>| async move {
      request
        .extensions()
        .get::<crate::ext::InformationalSender>()
        .expect("request informational sender")
      ...
      let frame = request
        ...
        .expect("valid request body frame");
      assert_eq!(frame.into_data().expect("request body data"), "xy");
      Ok::<_, Infallible>(crate::Response::new(Empty::<Bytes>::new()))
    });
    let mut builder = crate::server::conn::http1::Builder::new();
    builder.auto_date_header(false);
    ...
    client_io
      ...
      .await
      .expect("write HTTP/1 request head");
    let interim = read_http1_head(&mut client_io).await;
    assert!(interim.starts_with("HTTP/1.1 104 "), "{interim:?}");
    client_io
      .write_all(b"xy")
      .await
      .expect("write delayed HTTP/1 request body");
    let final_response = read_http1_head(&mut client_io).await;
  ...
  #[tokio::test]
  async fn relayed_continue_before_body_read_suppresses_later_automatic_continue() {
    let (server_io, mut client_io) = tokio::io::duplex(4096);
    let service = service_fn(|request: crate::Request<IncomingBody>| async move {
      request
        .extensions()
        .get::<crate::ext::InformationalSender>()
        .expect("request informational sender")
        .try_send(informational_response(100))
        .expect("queue relayed continue");
      Ok::<_, Infallible>(crate::Response::new(Empty::<Bytes>::new()))
    });
    let mut builder = crate::server::conn::http1::Builder::new();
    builder.auto_date_header(false);
    ...
    server
      ...
      .expect("HTTP/1 server connection");
    let wire = String::from_utf8(wire).expect("HTTP/1 responses are UTF-8");
    assert_eq!(wire.matches("HTTP/1.1 100 Continue").count(), 1, "{wire:?}");
  }

  #[tokio::test]
  async fn automatic_continue_before_relay_suppresses_later_relayed_continue() {
    let (server_io, mut client_io) = tokio::io::duplex(4096);
    let service = service_fn(|mut request: crate::Request<IncomingBody>| async move {
      let sender = request
        .extensions()
    ...
    client_io
      ...
      .await
      .expect("write HTTP/1 request head");
    let automatic = read_http1_head(&mut client_io).await;
    assert!(automatic.starts_with("HTTP/1.1 100 Continue"));
    client_io
      .write_all(b"xy")
      .await
      .expect("write delayed HTTP/1 request body");
