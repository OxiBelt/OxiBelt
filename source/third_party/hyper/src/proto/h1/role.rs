...
impl Http1Transaction for Server {
  ...
  fn is_continue_response(message: &Self::Outgoing) -> bool {
    *message == StatusCode::CONTINUE
  }
    ...
    for header in &headers_indices[..headers_len] {
      ...
      match name {
        ...
        header::CONTENT_LENGTH => {
          ...
          let len =
            headers::content_length_parse(&value).ok_or_else(Parse::content_length_invalid)?;
  ...
  fn encode(mut msg: Encode<'_, Self::Outgoing>, dst: &mut Vec<u8>) -> crate::Result<Encoder> {
    ...
    let (ret, is_last) = if msg.head.subject == StatusCode::SWITCHING_PROTOCOLS {
      ...
      && msg
        .head
        .extensions
        .get::<crate::ext::ServerInformational>()
        .is_some()
    {
      (Ok(()), false)
    } else if msg.head.subject.is_informational() {
    ...
    let encoder = if let Some(orig_headers) = orig_headers {
      Self::encode_headers_with_original_case(msg, dst, is_last, orig_len, wrote_len, orig_headers)?
    } else {
...
impl Server {
  ...
  fn encode_headers_with_lower_case(
    ...
    impl HeaderNameWriter for LowercaseWriter {
      #[inline]
      fn write_full_header_line(&mut self, dst: &mut Vec<u8>, line: &str, _: (HeaderName, &str)) {
  ...
  fn encode_headers_with_original_case(
    ...
    impl HeaderNameWriter for OrigCaseWriter<'_> {
      ...
      #[inline]
      fn write_header_name_with_colon(&mut self, dst: &mut Vec<u8>, _: &str, name: HeaderName) {
        self.write_header_name(dst, &name);
        extend(dst, b": ");
      }
      ...
      fn write_header_name(&mut self, dst: &mut Vec<u8>, name: &HeaderName) {
        ...
        let (_, values) = current.get_or_insert_with(|| (name.clone(), map.get_all_internal(name)));
    ...
    'headers: for (opt_name, value) in msg.head.headers.drain() {
      ...
      match *name {
        ...
        header::TRAILER => {
          ...
          if !is_name_written {
            is_name_written = true;
            header_name_writer.write_header_name_with_colon(dst, "trailer: ", header::TRAILER);
            extend(dst, value.as_bytes());
          } else {
            extend(dst, b", ");
            extend(dst, value.as_bytes());
...
impl Http1Transaction for Client {
  ...
  fn is_continue_response(_: &Self::Outgoing) -> bool {
    false
  }
  ...
  fn encode(msg: Encode<'_, Self::Outgoing>, dst: &mut Vec<u8>) -> crate::Result<Encoder> {
    ...
    *msg.req_method = Some(msg.head.subject.0.clone());
    ...
    if let Some(orig_headers) = msg.head.extensions.get::<HeaderCaseMap>() {
      write_headers_original_case(&msg.head.headers, orig_headers, dst, msg.title_case_headers);
