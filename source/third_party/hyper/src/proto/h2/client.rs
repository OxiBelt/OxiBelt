    ...
    loop {
      match this.fut.poll_informational(cx) {
        Poll::Ready(Some(Ok(response))) => {
          if let Some(callback) = this.informational.as_ref() {
            callback.call(response);
          }
        }
        Poll::Ready(Some(Err(error))) => {
          return Poll::Ready(Err((crate::Error::new_h2(error), None::<Request<B>>)));
        }
    ...
    loop {
      ...
      match self.req_rx.poll_recv(cx) {
        Poll::Ready(Some((req, cb))) => {
          ...
          if is_connect
            && headers::content_length_parse_all(req.headers()).map_or(false, |len| len != 0)
          {
...
mod informational_tests {
  ...
  #[tokio::test]
  async fn informational_callback_is_optional_and_preserves_final_response() {
    ...
    let server = tokio::spawn(async move {
      ...
      for _ in 0..2 {
        let (_request, mut respond) = connection
          .accept()
          .await
          .expect("HTTP/2 client closed early")
        ...
        respond
          ...
          .expect("send HTTP/2 informational response");
        respond
          .send_response(Response::new(()), true)
          .expect("send HTTP/2 final response");
      }
      tokio::select! {
        _ = done_rx => {}
        _ = connection.accept() => {
          panic!("unexpected third HTTP/2 request");
        }
      }
    });

    let (mut client, connection) = crate::client::conn::http2::Builder::new(TokioExecutor)
      .handshake::<_, Empty<Bytes>>(Compat::new(client_io))
      .await
      .expect("Hyper HTTP/2 client handshake");
    let client_driver = tokio::spawn(async move { connection.await });

    let request = Request::builder()
      .uri("https://example.test/without-callback")
      .body(Empty::<Bytes>::new())
      .expect("HTTP/2 request");
    assert_eq!(
      tokio::time::timeout(Duration::from_secs(1), client.send_request(request))
        .await
        .expect("HTTP/2 response without callback timed out")
        .expect("HTTP/2 response without callback")
        .status(),
      StatusCode::OK
    );
    ...
    let mut request = Request::builder()
      .uri("https://example.test/with-callback")
      .body(Empty::<Bytes>::new())
      .expect("HTTP/2 request");
    crate::ext::on_informational(&mut request, move |response| {
      callback_statuses
        .lock()
    ...
    assert_eq!(
      tokio::time::timeout(Duration::from_secs(1), client.send_request(request))
        .await
        .expect("HTTP/2 response with callback timed out")
        .expect("HTTP/2 response with callback")
        .status(),
      StatusCode::OK
    );
    assert_eq!(
      *statuses.lock().expect("callback statuses lock"),
      [StatusCode::from_u16(104).expect("104 status")]
    );

    done_tx.send(()).expect("release HTTP/2 server");
    drop(client);
