      ...
      loop {
        ...
        match ready!(self.conn.poll_accept(cx)) {
          Some(Ok((req, mut respond))) => {
            ...
            let (informational, receiver) = informational_channel();
...
pin_project! {
        ...
        B: Body,
    {
        Service {
            #[pin]
            fut: F,
            informational: InformationalReceiver,
    ...
    loop {
      let next = match me.state.as_mut().project() {
        H2StreamStateProj::Service {
          ...
          loop {
            match informational.poll_recv(cx) {
              Poll::Ready(Some(response)) => {
                if let Err(error) = me.reply.send_informational(response) {
                  debug!("send informational response error: {}", error);
                  me.reply.send_reset(Reason::INTERNAL_ERROR);
                  return Poll::Ready(Err(crate::Error::new_h2(error)));
                }
              }
          ...
          let res = match h.poll(cx) {
            ...
            Poll::Pending => {
              ...
              if let Poll::Ready(reason) = me.reply.poll_reset(cx).map_err(crate::Error::new_h2)? {
          ...
          // The service can enqueue an informational response and return its
          // final response in one poll. Drain the bounded queue again before
          // sending the final HEADERS frame, preserving wire ordering.
          loop {
            match informational.poll_recv(cx) {
              Poll::Ready(Some(response)) => {
                if let Err(error) = me.reply.send_informational(response) {
...
mod tests {
  ...
  fn informational_response() -> Response<()> {
    Response::builder()
      .status(104)
  ...
  async fn server_writes_same_poll_informational_before_final_response() {
    ...
    let service = service_fn(|request: Request<IncomingBody>| async move {
      request
        .extensions()
        .get::<crate::ext::InformationalSender>()
        .expect("request informational sender")
        .try_send(informational_response())
        .expect("queue informational response");
      Ok::<_, Infallible>(Response::new(Empty::<Bytes>::new()))
    });
    let server = tokio::spawn(async move {
      crate::server::conn::http2::Builder::new(TokioExecutor)
    ...
    let (mut response, _) = client
      .send_request(request, true)
      .expect("send HTTP/2 request");
    let interim = tokio::time::timeout(
      Duration::from_secs(1),
      poll_fn(|cx| response.poll_informational(cx)),
    )
    .await
    .expect("HTTP/2 informational response timed out")
    .expect("HTTP/2 informational stream ended")
    .expect("HTTP/2 informational response");
    assert_eq!(interim.status().as_u16(), 104);
    assert_eq!(
      tokio::time::timeout(Duration::from_secs(1), response)
        .await
        .expect("HTTP/2 final response timed out")
        .expect("HTTP/2 final response")
        .status(),
      StatusCode::OK
    );
  ...
  async fn server_flushes_informational_while_upload_is_still_pending() {
    let (server_io, client_io) = tokio::io::duplex(4096);
    let service = service_fn(|mut request: Request<IncomingBody>| async move {
      request
        .extensions()
        .get::<crate::ext::InformationalSender>()
        .expect("request informational sender")
    ...
    let (mut response, mut upload) = client
      ...
      .expect("send HTTP/2 request head");
    let interim = tokio::time::timeout(
      Duration::from_secs(1),
      poll_fn(|cx| response.poll_informational(cx)),
    )
    ...
    assert_eq!(interim.status().as_u16(), 104);
    upload
      .send_data(Bytes::from_static(b"xy"), true)
      .expect("send delayed HTTP/2 request body");
    assert_eq!(
      tokio::time::timeout(Duration::from_secs(1), response)
        .await
        .expect("HTTP/2 final response timed out")
