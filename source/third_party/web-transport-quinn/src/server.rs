...
impl Request {
    ...
    pub async fn accept(conn: quinn::Connection) -> Result<Self, ServerError> {
        Self::accept_with_draft(conn, web_transport_proto::WebTransportDraft::Draft02).await
    }

    /// Accept using an explicitly selected HTTP/3 WebTransport dialect.
    pub async fn accept_with_draft(
        conn: quinn::Connection,
        ...
        if connect.request.draft != draft {
            return Err(ServerError::DraftMismatch);
        }
    ...
    pub async fn respond(
        ...
        let response = response.into().for_draft(self.connect.request.draft);
